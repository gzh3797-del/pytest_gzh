...
3. 按下方「生成前必做：读取当前项目知识库背景」章节确定目标项目，读取其 `context.md`，扫 `requirements/summaries/` 清单、`bugs/INDEX.md` 与 `testcase/` 用例摘要，并**输出「知识库提取表」**（起草前必做）
4. 阅读下方「句式模板与语言规范」章节
...
- **知识库是补充背景，不是权威来源**：项目 `context.md` / `requirements/summaries/` / `bugs/INDEX.md` / `testcase/` 摘要仅用于术语与模块/设备范围对齐、历史缺陷模式借鉴、已有覆盖增量判断等背景场景；待测功能仍以会话喂入的当前需求文档为唯一权威依据，**知识库与当前文档冲突时一律以当前文档为准**（知识库可能过时）
...
- **提取设备清单（必做）**：把项目的被测/下挂设备具体型号清单（如 AcuRev4100、AcuIOM-01~04）摘出备用；后续凡涉及设备的测试点必须落到具体型号，禁止"设备"泛称（硬性规则见「语言规范→格式规范」）
...
**4. 读 `bugs/INDEX.md`（历史缺陷模式）**
- 路径：项目知识库根（即 `context.md` 所在目录）下 `bugs/INDEX.md`；只读精简索引，不翻 `bugs/raw/` 原件
- 用途：作为第四步「错误推测」的输入——bug 集中的模块与缺陷类型（如单位换算、字节序、断电恢复、保存不生效）优先补测试点；与当前功能直接相关的已知 bug 场景写成回归类测试点（句首加 `[补充]`，可注明来源 bug ID）

**5. 扫 `testcase/` 用例摘要（已有覆盖）**
- 路径：项目知识库根下 `testcase/` 内的摘要 `.md`；无摘要时仅列文件名判断相关性，不解析 xlsx 原件
- 用途：了解该功能/模块已有用例覆盖范围，本次输出聚焦增量、避免与已有用例语义重复；模块划分与术语与已有用例保持一致
...
- 定位：其他项目知识库根 `testcase/` 摘要中该模块的用例覆盖维度，作为**测试维度基线**
- 用法：基线维度逐条三分类——`适用`（作为测试点纳入，设备/参数/端口按本项目必读区适配）/ `不适用`（记原因）/ `待确认`（挂「待确认」交人工裁决）；来自基线的补充点句首加 `[补充]` 前缀
- 硬约束：**当前需求文档仍是唯一权威**——基线只用于补维度防漏（先覆盖、再添加），需求中不存在的功能不得因基线而生成测试点

**7. 兜底**
- 该项目无 `context.md`，或 `requirements/summaries/`、`bugs/INDEX.md`、`testcase/` 为空/不存在时，对应项跳过并一句话说明，照常用会话喂入的文档生成

**8. 输出「知识库提取表」（起草前必做，不得跳过）**

完成上述读取后、起草任何测试点之前，必须在对话中输出下表，把"读到了什么"显性化为可核对的中间产物：
...
| 容差与比对约定 | 数值比对容差等项目约定 |
| 历史缺陷模式 | bugs/INDEX.md 中与当前功能相关的高频缺陷 |
| 已有用例覆盖 | testcase/ 摘要中同模块已覆盖范围 |
...
- 表中事实必须给出来源（context.md / INDEX.md / 摘要文件名）
- 后续起草的测试点凡涉及表中类别，必须与表中事实一致（如设备遍历必须覆盖设备清单全部型号）
- 🔴 **CHECKPOINT（工作流 gate）**：提取表随脑图产物一起交人工审核，供审核人核对"读全了没有"
...
- **错误推测**：基于工程经验/历史 bug 模式推测的易错点（如单位换算、字节序、时间戳/时区）；优先取「生成前必做」读到的项目 `bugs/INDEX.md` 高频缺陷模块与缺陷类型，相关已知 bug 场景写成回归测试点
...
执行中命中以下触发条件时，按表处理，禁止静默跳过或自行臆测：

| 触发条件 | 一线处理 | 仍失败兜底 |
|----------|----------|------------|
| 目标项目推断不出或有歧义 | 反问用户确认 | 用户未答复 → 停在此步，不臆测生成 |
| `context.md` / `summaries/` / `bugs/INDEX.md` / `testcase/` 缺失或为空 | 对应项跳过并一句话说明 | 「知识库提取表」对应类别写 `无`，不留空行 |
| docx 正文提取不到内容（文字在图片里） | 用 zipfile 解包读取 `word/media/` 图片后识别 | 图片仍不可读 → 对应测试点写 `（需确认）` 占位并告知用户 |
| 需求未提供功能概述某项信息 | 写 `（需确认）` 占位 | 不得省略该子节点 |
| 生成的 .xmind 用 XMind 26 打不开 | 先查 `META-INF/manifest.xml` 是否缺失 | 再查 `structure-class` / style-id / 特殊字符 escape，修复后重新打包验证 |
| 分组/根节点预估数与叶子标注对不上 | 以叶子「X个用例」求和为准重算 | 未标注的叶子按 1 条计 |
| coverage-check 报出 🔴/🟡 漏点 | 原样列给工程师，由其决定是否回炉 | 不自动改产物、不代替工程师裁决 |
...
- [ ] 已读 `bugs/INDEX.md`，高频缺陷模式已纳入错误推测补充点；已扫 `testcase/` 摘要，本次输出为增量、未与已有用例语义重复（项目无对应内容时已说明跳过）
- [ ] 涉及设备的测试点已全部落到具体型号（对照 `context.md` 设备清单），无"设备/下挂设备/电表"泛称；遍历类已列全型号与用例数
- [ ] 已在起草前输出「知识库提取表」，九类全部填写（无内容的写"无"）且注明来源；测试点与表中事实一致
- [ ] 存在跨项目同源模块时：基线维度已逐条三分类（先覆盖再添加），适用维度均有对应测试点，需求外功能未因基线混入
...
- **设备型号具体化（硬性）**：凡涉及被测/下挂设备的测试点，必须写具体型号（对照「生成前必做」提取的设备清单，如 `AcuRev4100 / AcuIOM-01`），**禁止"设备""下挂设备""电表"等泛称**；确与型号无关的测试点，须写明"任一型号"并给出理由
...
| 选择设备，查看数据上报 | 选择 AcuRev4100，查看数据上报；遍历 AcuRev4100 / AcuIOM-01 / AcuIOM-03（对照 context.md 设备清单列全），3个用例 |
