...
import logging
import ast
from comm.device_reboot import *

test_data_path = r'/Convenient_tools/poweroff_poweron_seting_check/data/checking_data.xlsx'
...
if __name__ == '__main__':
    ...
    """
    print(f"====================Power_OFF and power_ON Measure Start====================")
    ...
    for i in range(100):
        print(f"第{i+1}次循环")
        diff_final = script_start.select_project(project_name)  # 调用 select_project 方法并传入参数
    print(f"配置发生变化的次数：{diff_final}")
    print(f"====================测试总耗时:{time.time() - start_time}====================")
    print(f"====================={time.strftime('%Y_%m_%d %H:%M:%S')}=====================")
    print(f"====================Power_OFF and power_ON Measure End====================")
