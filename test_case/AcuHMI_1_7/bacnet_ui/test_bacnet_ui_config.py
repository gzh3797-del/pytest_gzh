...
def _navigate_to_bacnet(page: Page) -> None:
    ...
    page.evaluate(
        """() => {
            const nav = document.querySelector('.nav-item:has-text, .nav-item-active, .nav-item');
            // 找文字为 AcuHMI-1-7 的导航项
            for (const el of document.querySelectorAll('.nav-item')) {
                if (el.textContent.includes('AcuHMI-1-7')) { el.click(); return; }
            }
        }"""
    )
    page.wait_for_timeout(1500)
...
class TestBACnetUIConfig:
    ...
    def test_033_cov_batch_update_4100_save(self, hmi_page: Page) -> None:
        ...
        """
        devices = _get_device_names(hmi_page)
        keyword_4100 = next((d for d in devices if "4100" in d), None)
        if keyword_4100 is None:
            pytest.skip(
                f"AcuRev-4100 设备未接入（当前设备列表：{devices}），跳过此用例"
            )
    ...
    def test_029_empty_cov_increment_blocks_param_type_switch(self, hmi_page: Page) -> None:
        ...
        if not _open_param_dialog(hmi_page, devices[0]):
            pytest.skip(f"无法打开设备 {devices[0]!r} 的 Parameter Config 弹窗")
    ...
    def test_032_cov_batch_update_2100_save(self, hmi_page: Page) -> None:
        ...
        """
        devices = _get_device_names(hmi_page)
        keyword_2100 = next((d for d in devices if "2100" in d), None)
        if keyword_2100 is None:
            pytest.skip(
                f"AcuRev-2100 设备未接入（当前设备列表：{devices}），跳过此用例"
            )
