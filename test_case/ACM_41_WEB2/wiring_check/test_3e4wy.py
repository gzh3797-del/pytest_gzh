...
对比逻辑：
  ...
  Actual   — 由 Playwright 从 WEB2 页面 Wiring Status 列读取
...
from test_case.ACM_41_WEB2.wiring_check.core import config as cfg
from test_case.ACM_41_WEB2.wiring_check.core.meter_modbus import WiringCheckModbus
from test_case.ACM_41_WEB2.wiring_check.core import signal_driver as src
from test_case.ACM_41_WEB2.wiring_check.core.expected_engine import compute_3e4wy
from test_case.ACM_41_WEB2.wiring_check.core.wiring_check_page import WiringCheckPage
from test_case.ACM_41_WEB2.wiring_check.core import report as rpt
from test_case.ACM_41_WEB2.wiring_check.core.tc_map import TC_MAP_3E4WY as _TC_MAP

logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(message)s')

V = cfg.NOMINAL_VOLTAGE   # 230
I = cfg.NORMAL_CURRENT    # 1.0

# ── 设备名称（Gateway 模式下页面 Device 选择器中显示的名称）─────────────────
DEVICE_NAME = cfg.WEB2_DEVICE_NAME
...
#   id          : 编号
#   desc        : 描述
#   phase_order : PHASE_ABC / PHASE_ACB
#   src         : 控源参数 dict
# ─────────────────────────────────────────────────────────────────────────────
...
ABC_TESTS = [
    ...
    # ── 条件2/3/4 guard 负向用例：guard 不成立时不误报双相缺失 ──
...
ACB_TESTS = [
    ...
    # ── 电流侧 ACB（B/C 相 PF 依赖 Vb/Vc 角度，需独立 ACB 用例）──
...
# 用例 → 接线检测总表_ver1.05.xlsx 行号对照（Sheet: 3LN接线(3E4W Y)）
_SPEC_REFS = {
    'PASS-ABC': '—',        'PASS-ACB': '—',
    'V-01-ABC': '3LN接线, 行2',   'V-01-ACB': '3LN接线, 行2',
    'V-02-ABC': '3LN接线, 行3',
    'V-03-ABC': '3LN接线, 行4',
    'V-04-ABC': '3LN接线, 行5',
...
def _compare_current(expected: dict, actual_list: list[dict],
    ...
    """
    所有 Meter Point 的各相状态都应与 expected 一致。
    若某 User Channel 的某相实测为空（该 Channel 未分配该相），跳过该相比对。

    channel_phase_map: 可选，每个 channel 实际分配的相元组列表，
                       例如 (('A','B'), ('C','A'), ('B','C')) * 4（2E3W Network）。
                       不传则对所有 channel 检查 A/B/C 三相。
    """
    diffs = []
...
def _status_match(expected: str, actual: str) -> bool:
    """
    宽松比对：去空格、小写、允许 UI 使用缩写。
    e.g. 'Wiring Missing' 匹配 'Va Wiring Missing' / 'Missing'
    expected 为空或 'N/A' 表示该相不参与检测，直接视为匹配。
    """
    e = expected.lower().strip()
    a = actual.lower().strip()
    if not e or e == 'n/a':
        return True   # 无预期值（不检测该相），始终匹配
    if e == a:
        return True
    # 允许 UI 只显示关键词
...
def run_one(tc: dict, modbus: WiringCheckModbus, wc_page: WiringCheckPage) -> dict:
    ...
    # 1. 配置表寄存器（相序每条独立写；接线方式和额定电压只在 setup 写一次）
    modbus.write_phase_order(tc['phase_order'])

    # 2. 控源输出（set_ac 内部有 20s 稳定等待）
...
def run_all(tests: list[dict] = ALL_TESTS, headless: bool = False):
    ...
    try:
        ...
        src.stop()
        modbus.write_phase_order(cfg.PHASE_ABC)  # 恢复默认相序 ABC
        modbus.close()
...
if __name__ == '__main__':
    # 不传参数：跑全部用例
    # 传 ID 参数：只跑指定用例，例如：
    #   python test_3e4wy.py V-13-ABC
    #   python test_3e4wy.py I-04-ABC I-07-ABC
    _ids = sys.argv[1:]
    if _ids:
    ...
    # run_all(tests=VOLTAGE_TESTS)  # 只跑电压
    # run_all(tests=CURRENT_TESTS)  # 只跑电流
