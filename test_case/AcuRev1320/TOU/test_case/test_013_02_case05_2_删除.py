...
def test_013_02_case5_1():
    ...
    # 配置4个session,对应Schedule Id 1
    tou.tou_seasons_add(session_id=4, start_date=start_date, seasons_schedule_id=seasons_schedule_id)
    # 上位机上点击update
    update_success = tou.click_update()
    assert update_success
    # 上位机上删除3个seasons
    tou.tou_seasons_remove(number=3)
    update_success = tou.click_update()
    assert update_success
