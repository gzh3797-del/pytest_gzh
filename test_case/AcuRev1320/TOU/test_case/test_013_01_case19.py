...
"""
Function_AcuRev1320_013_01_case19
"""

dst_format = 2
(start_month, start_day, start_week, start_time, start_adjust_time) = (3, 1, 6, '00:00', 60)
(end_month, end_day, end_week, end_time, end_adjust_time) = (6, 1, 2, '03:00', 60)
(end_month2, end_day2, end_week2, end_time2, end_adjust_time2) = (7, 1, 2, '03:00', 60)
time_year, time_month, time_day, time_hour, time_minute, time_second = 2025, 7, 7, 2, 59, 55
...
def test_013_01_case19():
    ...
    # 上位机设置format2_DST 结束time2和adjust_time2
    tou.setting_format2_DST_time(start_month=start_month, start_day=start_day, start_week=start_week,
                                 start_time=start_time, start_adjust_time=start_adjust_time,
                                 end_month=end_month2, end_day=end_day2, end_week=end_week2,
                                 end_time=end_time2, end_adjust_time=end_adjust_time2)
    logging.info(f'上位机设置DTS format2 开始时间:{start_month}月、{start_day}周、星期{start_week - 1}, '
                 f'DTS开始时间{start_time}、DTS开始调整时间{start_adjust_time}'
                 f'上位机设置DTS format2 结束时间:{end_month2}月、{end_day2}周、星期{end_week2 - 1}, '
                 f'DTS结束时间{end_time2}、DTS结束调整时间{end_adjust_time2}'
                 )
    # 上位机点击update，检查是否更新成功
    update_success = tou.click_update()
    assert update_success
