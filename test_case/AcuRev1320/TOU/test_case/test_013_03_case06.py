...
"""
    Special Weekday Schedule栏，删除2个已添加的season（3个）成功，update保存设置成功。
    
    1、连接AcuRev1320电表到Acuview2上，连接状态为connected。
    2、Special Weekday Schedule栏，删除2个已添加的season
    3、上位机上点击"Update"。
    4、检查步骤2-3中的配置是否保存成功。
"""
...
def test_013_03_case1():
    ...
    tou.tou_seasons_add(session_id=3, seasons_schedule_id=[1, 1, 1])
    # Special Weekday Schedule栏，已存在season1配置Schedule2(勾选Mon、Tue、Wed、Thu、Fri、Sat Sun，Sat）
    tou.tou_special_weekday_schedule_add(session_id=3,
                                         weekend_selection=[[0], [0], [0]],
                                         weekend_schedule=[[1], [1], [1]])
    # enable_special_weekday_schedule使能打开
    tou.open_enable_special_weekday_schedule()
    # 上位机上点击update
    update_success = tou.click_update()
    assert update_success
    # Special Weekday Schedule栏，删除2个已添加的season
    tou.tou_special_weekday_schedule_remove(number=2)
    update_success = tou.click_update()
    assert update_success
