...
"""
Function_AcuRev1320_013_02_case5
    删除2个TOU Season(3个)，update 更新保存正常
"""
...
def test_013_02_case5_1():
    ...
    # 配置2个session,对应Schedule Id 1
    tou.tou_seasons_add(session_id=3, start_date=start_date, seasons_schedule_id=seasons_schedule_id)
    # 上位机上点击update
    update_success = tou.click_update()
    assert update_success
    # 上位机上删除一个seasons
    tou.tou_seasons_remove(number=2)
    update_success = tou.click_update()
    seasons = tou.check_number_of_seasons(number=1)
    assert seasons
