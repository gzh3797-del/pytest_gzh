...
from test_case.AcuRev1320.power_quality.sequence_memory_addrs import MemoryAddr, SlaveId, MemoryReg
...
class HandleMemory:
    ...
    def read_voltage_unbalance_negative(self):
        """
    ...
    def read_current_unbalance_negative(self):
        """
    ...
    def read_voltage_zero_sequence(self):
        """

        :return: zero_sequence：real, imag, z_magnitude, z_angle
        """
        ...
        for i in range(2):
            i = i * 2
            reg = hex(measure_value[i]).replace('0x', '').zfill(4) + hex(measure_value[i + 1]).replace('0x', '').zfill(
                4)
        ...
        value_real = read_list[0]
        value_imag = read_list[1]
        value_complex = complex(value_real, value_imag)
        value_magnitude = abs(value_complex)
        value_angle = round(self.calculate_angle(value_complex, value_magnitude), 3)
        return value_real, value_imag, value_magnitude, value_angle

    def read_voltage_positive_sequence(self):
        """
    ...
    def read_voltage_negative_sequence(self):
        """
    ...
    def read_current_zero_sequence(self):
        """
    ...
    def read_current_positive_sequence(self):
        """
    ...
    def read_current_negative_sequence(self):
        """
    ...
    @staticmethod
    def get_measure_accuracy_by_vuf(standard_value, measure_values):
        """
        获取电压/电流/功率精度计算结果
        :param standard_value: sequence值
        :param measure_values: 寄存器测量值
        :return: sequence计算结果
        """
        min_val = min(measure_values)
    ...
    def set_phase_order_by_voltage(self, phase_order=0):
        """
        设置电压接线方式
        :param voltage_wire_mode: 电压接线方式
        :return:
        """
        address = MemoryAddr.phase_order_addr
        values = phase_order
        slave = self.slave_id
        count = MemoryReg.reg_single
        ret = self.modbus_client.write_registers(address=address, values=values, slave=slave)
...
if __name__ == '__main__':
    rm = HandleMemory()
    r = rm.read_voltage_zero_sequence()
    print(r)
