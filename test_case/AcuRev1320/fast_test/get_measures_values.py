...
test_case_path = r'./test_case/AcuRev1320/fast_test/rev1320_test_case.xlsx'
...
class PrecisionMeasure:
    ...
    @staticmethod
    def write_table_title_of_3e4wy_to_excel_by_measures(file_path, wb, ws):
        """
        写入表头:3e4wy
        :param file_path: 文件路径
        :param wb: 工作簿对象
        ...
        ret = [
            ...
            "ua(V)", "ub(V)", "uc(V)",
            "ia(A)", "ib(A)", "ic(A)",
            "ua_ang(°)", "ub_ang(°)", "uc_ang(°)",
            "ia_ang(°)", "ib_ang(°)", "ic_ang(°)",
            "pa_power(kW)", "pb_power(kW)", "pc_power(kW)"
        ]
    ...
    def get_measure_values_of_freq(self):
        """
        获取相电压测量值
        :return: 相电压测量值
        """
        freq = self.handle_memory.read_frequency()
        return freq
    ...
    def get_measure_values_by_sample_cnt_of_3e4wy(self, sample_cnt, sample_interval):
        ...
        :return: 测量值
        """
        measure_values_of_time = []
        measure_values_of_freq = []
        ...
        for j in range(sample_cnt):
            ...
            record_time_measure = stamp.strftime("%Y%m%d:%H%M%S")
            freq_measure = self.get_measure_values_of_freq()
            ...
            measure_values_of_time.append(record_time_measure)
            measure_values_of_freq.append(freq_measure)
    ...
    def fast_precision_measure_by_3e4wy(self, file_path, input_list):
        ...
        ws = wb.active
        self.write_table_title_of_3e4wy_to_excel_by_measures(file_path, wb, ws)
        for i in range(len(input_list)):
            ...
            (pa, _, _) = self.get_power_cal_res_of_phase(ua, ia, ua_p, ia_p)
            (pb, _, _) = self.get_power_cal_res_of_phase(ub, ib, ub_p, ib_p)
            (pc, _, _) = self.get_power_cal_res_of_phase(uc, ic, uc_p, ic_p)
            stamp_1st = datetime.datetime.now()
            record_time_1st = stamp_1st.strftime("%Y%m%d:%H%M%S")
            start_num = 1
            common_values_of_3e4wy = (
                record_time_1st, freq, ua, ub, uc, ia, ib, ic, ua_p, ub_p, uc_p, ia_p, ib_p, ic_p, pa, pb, pc
            )
            ...
            print(f"cost get_datas_time:{time.time() - get_time}")
            measure_values = list(chain.from_iterable(zip(
                measure_values_of_time,
                measure_values_of_freq,
                measure_values_of_ua,
                measure_values_of_ub,
            ...
            for row in zip_longest(*[iter(measure_values)] * 17, fillvalue=""):
                ws.append(row)
            wb.save(file_path)
            print(f"cost write_excel_time:{time.time() - write_time}")
    ...
    def select_wire_type(self, case_path, sheet_name, wire_type):
        ...
        if wire_type == 0:
            ...
            input_list_of_wire_type = self.get_input_list_by_wire_type(source_input_list, wire_type)
            input_list = self.get_real_time_parameters_by_input_list_of_wire_type(input_list_of_wire_type)
            # file_path：'C:\\autotest_local\\autotest\\test_case\\AcuRev1320\\fast_test\\precision_measure_20251224\\Precision_Measure_3E4WY_20251224103110.xlsx'
            # save_filedir： C:\autotest_local\autotest\test_case\AcuRev1320\fast_test\precision_measure_20251224
