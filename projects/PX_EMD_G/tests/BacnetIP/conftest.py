...
# ── 设备 Web UI 配置（统一维护于 projects/PX_EMD_G/settings.py）────────────
from projects.PX_EMD_G.settings import get_screenshot_dir  # noqa: E402
from projects.PX_EMD_G.pages.login_page import LoginPage  # noqa: E402
...
def _login_and_navigate_bacnet(page: Page) -> None:
    ...
    # ── 切换到 PX-EMD-G 产品管理菜单（JS click，绕过 overlay / actionability 检查）──
    # 注意：顶部存在两处 "PX-EMD-G" 文本——左上角纯展示的 header-title 与可点击的 .nav-item
    # 产品菜单。这里只遍历 .nav-item 并按 trim 后精确匹配，避免误点不可导航的 header-title。
    page.evaluate(
        """() => {
