...
pytest projects/PX_EMD_G/tests/BacnetIP/ -v --html=reports/bacnet_ui.html --self-contained-html

# 只跑 P0 参数列表一致性（纯 UI，无 BACnet 客户端）
pytest projects/PX_EMD_G/tests/BacnetIP/test_bacnet_ui_basic.py -v --html=reports/bacnet_ui.html --self-contained-html

# 只跑协议元数据/合规/稳定性 050–054（需 BACnet 客户端，须串行执行勿加 -n）
pytest projects/PX_EMD_G/tests/BacnetIP/test_bacnet_six_segment.py -v --html=reports/bacnet_ui.html --self-contained-html

# 只跑 P1/P2 配置页面功能（纯 UI，无 BACnet 客户端）
pytest projects/PX_EMD_G/tests/BacnetIP/test_bacnet_ui_config.py -v --html=reports/bacnet_ui.html --self-contained-html

# 只跑协议端到端验证（需 BACnet 客户端连接，须串行执行勿加 -n）
pytest projects/PX_EMD_G/tests/BacnetIP/test_bacnet_ui_protocol.py -v --html=reports/bacnet_ui.html --self-contained-html

# 单条用例（-k 关键词过滤）
pytest projects/PX_EMD_G/tests/BacnetIP/ -k "test_013" -v --html=reports/bacnet_ui.html --self-contained-html

# 多条用例（-k 用 or 连接多个关键词；适合挑几条一起跑）
pytest projects/PX_EMD_G/tests/BacnetIP/ -k "test_021 or test_033 or test_046" -v --html=reports/bacnet_ui.html --self-contained-html
```
