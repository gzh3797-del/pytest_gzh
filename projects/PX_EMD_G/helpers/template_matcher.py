...
# PX-EMD-G 网关用 PX 代号 + 变体后缀上报 deviceModel（实测：
#   PXB-M24-XMA-GEN / PXB-M24-XMV-GEN = AcuRev4100；PXE2 = AcuvimIIW；
#   PXE1 = AcuvimIIR；PXM350 = AcuRev1300）。后缀随硬件配置（电压/电流输入、通道数）
# 变化，故按「归一化前缀」匹配，不能用全串精确匹配。同族设备名（AcuvimIIR/IIW）
# 名字关键词子串无法区分，必须靠此 Model 前缀解析。
# 顺序按更具体的前缀在前，避免短前缀抢先误命中。
_PX_MODEL_PREFIX_TO_TEMPLATE: list[tuple[str, str]] = [
    ("pxm350", "AcuRev1300"),
    ("pxe1",   "AcuvimIIR"),
    ("pxe2",   "AcuvimIIW"),
    ("pxb",    "AcuRev4100"),
...
def _normalize_model(model: str) -> str:
    """归一化型号串：去连字符/空格、转小写，供大小写/连字符不敏感匹配。"""
    return model.replace("-", "").replace(" ", "").lower()


# 预归一化精确映射。
_NORMALIZED_MODEL_TO_TEMPLATE: dict[str, str] = {
    _normalize_model(model): tmpl for model, tmpl in MODEL_TO_TEMPLATE.items()
}
...
def resolve_template_from_model(model: str) -> Optional[str]:
    ...
    先按归一化全串精确匹配（HMI 历史命名），再按 PX 代号前缀匹配
    （PX-EMD-G 的 deviceModel 形如 'PXB-M24-XMA-GEN'，后缀随硬件变化）。
    """
    if not model:
        return None
    key = _normalize_model(model)
    exact = _NORMALIZED_MODEL_TO_TEMPLATE.get(key)
    if exact:
        return exact
    for prefix, tmpl in _PX_MODEL_PREFIX_TO_TEMPLATE:
