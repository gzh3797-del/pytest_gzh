...
from projects.PX_EMD_G.helpers.hmi_bacnet_client import (
    DeviceInfoResult,
    MetadataItem,
    ProtocolCheckItem,
...
def get_bacnet_report_dir() -> Path:
    ...
    reports/PX_EMD_G/BacnetIP/<时间戳>/，结构与正式运行保持一致；
    ...
    if report_dir:
        ...
        base = _REPO_ROOT / "reports" / "PX_EMD_G" / "BacnetIP" / ts
...
<tbody>{"".join(val_rows)}</tbody>
    ...
    if dev_info is not None:
        ...
        dev_rows = "".join(
            ...
            f'<td class="val">{html.escape(str(v)) if v else "<em style=\'color:#721c24\'>未获取</em>"}</td></tr>'
