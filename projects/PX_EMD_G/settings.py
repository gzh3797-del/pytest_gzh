...
def get_screenshot_dir() -> Path:
    ...
    否则回退到 reports/PX_EMD_G/_adhoc。仅在真正要截图时调用、用时才 mkdir，
    避免 import settings 就在磁盘上凭空建目录。
    """
    path = Path(os.getenv("SCREENSHOT_DIR",
                          str(_REPO_ROOT / "reports" / "PX_EMD_G" / "_adhoc")))
    path.mkdir(parents=True, exist_ok=True)
    return path

# ── HMI 设备常量（迁移自 test_case/PX_EMD_G/config.py）──
...
_MQTT_CERT_DIR       = _REPO_ROOT / "projects" / "PX_EMD_G" / "tests" / "mqtt" / "certs"
...
# ── 多项目切换（只改 PROJECT 这一行即可）────────────────────────────────────
PROJECT = "HMI17"   # "WEB2" | "HMI17"

_PROJECTS = {
    "WEB2": {
        "url":    "https://192.168.3.56",
    ...
    "HMI17": {
        ...
        "user":   HMI_USERNAME,
        "pass":   HMI_PASSWORD,
        "device": MQTT_DEFAULT_DEVICE,
    },
}

# ── MQTT comparator 兼容别名（mqtt_comparator.py 通过 config.XXX 读取） ──────
GATEWAY_WEB_URL  = _PROJECTS[PROJECT]["url"]
GATEWAY_WEB_USER = _PROJECTS[PROJECT]["user"]
GATEWAY_WEB_PASS = _PROJECTS[PROJECT]["pass"]
DEVICE_NAME         = _PROJECTS[PROJECT]["device"]
MQTT_DEFAULT_DEVICE = _PROJECTS[PROJECT]["device"]   # 覆盖上面从 config.yaml 读的值
...
REPORT_DIR       = str(_REPO_ROOT / "reports" / "PX_EMD_G" / "mqtt")
