r"""Function_AC meter_010_13_case5
用例标题: 相序算法 Step2 对称性 ±10% 边界（以 Vc 承载）
...
3. 修改程控源输出（刚触发侧）：uc=188V（ua=ub=220V，偏差>平均值×10%）；关→开接线检查开关触发立即检测（或等待 ≥60s）
4. 经 Acuview2 Reading—接线检查 页与 Modbus 回读接线检查结果
5. 修改程控源输出（刚不触发侧）：uc=190V（偏差<平均值×10%）；触发立即检测后回读接线检查结果
6. 程控源还原步骤 2 正常基准值；触发立即检测后回读确认告警全部清除
预期结果: 2. 接线检查结果全部正常
4. 相序算法 Step2 失败，voltagePhaseOrder=2（以该寄存器值为断言）；输出 2 不参与条件13 比较，无任何接线告警
5. 对称性通过，voltagePhaseOrder=0，接线检查结果正常
6. 告警清除，接线检查结果恢复全部正常
...
def test_010_13_case5():
    report = run_wiring_case(case_meta={
        '编号': 'Function_AC meter_010_13_case5',
        '标题': '相序算法 Step2 对称性 ±10% 边界（以 Vc 承载）',
        '预置条件': '1、Acuview2上位机（RS-485，Modbus RTU）\n2、可程控三相功率源\n3、AcuRev-100电表\n4、接线检查功能已启用（默认启用）\n5、Dip Switch 处于解锁状态（允许接线配置写入）',
        '测试步骤': '1. 配置接线方式 = 3E4WY，相序配置 = ABC（Modbus 寄存器 0x1063 Phase Order = 0）\n2. 程控源输出正常基准：ua=ub=uc=220V，qua=0°/qub=240°/quc=120°，ia=ib=ic=5A，qia=0°/qib=240°/qic=120°，freq=50Hz；关→开一次接线检查开关触发立即检测（开关开启瞬间检查一次；常开时检测周期 1 次/min），回读接线检查结果确认全部正常\n3. 修改程控源输出（刚触发侧）：uc=188V（ua=ub=220V，偏差>平均值×10%）；关→开接线检查开关触发立即检测（或等待 ≥60s）\n4. 经 Acuview2 Reading—接线检查 页与 Modbus 回读接线检查结果\n5. 修改程控源输出（刚不触发侧）：uc=190V（偏差<平均值×10%）；触发立即检测后回读接线检查结果\n6. 程控源还原步骤 2 正常基准值；触发立即检测后回读确认告警全部清除',
        '预期结果': '2. 接线检查结果全部正常\n4. 相序算法 Step2 失败，voltagePhaseOrder=2（以该寄存器值为断言）；输出 2 不参与条件13 比较，无任何接线告警\n5. 对称性通过，voltagePhaseOrder=0，接线检查结果正常\n6. 告警清除，接线检查结果恢复全部正常',
