r"""Function_AC meter_014_01_case4
用例标题: 上位机通过RTU方式，波特率设置为57600,升级正常
预置条件: 1、Acuview2上位机 / 2、RTU串口线 / 3、AcuRev-100电表 / 4、铅封解锁
测试步骤: RTU连接; 记FW/配置基线; 波特率57600升级; 查无错/版本/数据保持
预期结果: 升级过程无错误; FW版本正确; 配置数据保持

生成说明: run_firmware_update_case(baud=57600)。判据(2026-07-15 裁决): 同版重刷判"升级流程成功", FW版本记录不强判(firmware.expect_version 填值后自动严判); 数据保持用配置类寄存器快照。🔴 升级会重启电表, 授权门禁 skip 兜底。
"""
...
def test_014_01_case4():
    report = run_firmware_update_case(
        case_meta={
            '编号': 'Function_AC meter_014_01_case4',
            '标题': '上位机通过RTU方式，波特率设置为57600,升级正常',
            '预置条件': '1、Acuview2上位机\n2、RTU串口线\n3、AcuRev-100电表\n4、铅封解锁',
            '测试步骤': 'RTU连接; 记FW/配置基线; 波特率57600升级; 查无错/版本/数据保持',
            '预期结果': '升级过程无错误; FW版本正确; 配置数据保持',
        },
        config_path=TEST_CONFIG, baud=57600,
