r"""Function_AC meter_004_01_case3
用例标题: CT选型333mV(200A)，一路电流为（100A），在上位机均配置为对应A相，测量Ia,Ib,Ic,Iavg满足精度0.2%
预置条件: 1、Acuview2上位机
2、RTU串口线
3、AcuRev-100电表
4、接线方式3E4WY
测试步骤: 1、连接AcuRev-100电表到Acuview2上，链接状态为connected
2、输入一路电流分别为（2.5A），在上位机均配置为对应A相
3、Acuview2上检查交流电相电流是否均在精度范围内
预期结果: 3、Acuview2上检查交流电相电流均在精度范围内
Ia: 99.800~100.200; Ib: 0~0; Ic: 0~0; Iavg: 33.267~33.400

生成说明: 方案A——CL3021 设源(0网段 UDP) + USB 口 Modbus 区间断言, 不驱动 Acuview2
GUI(故无锁屏 skipif); 判据/源设定见 case_map.yaml 同编号条目, 复核改 yaml 不改本文件。
2026-07-14 源点补齐(原xlsx解析丢源点): 手工2.5A按 mV→mA 换算×4 → ia=10A(显示100A),
ib/ic=0, 电压 220V 三相保活。
"""
from pathlib import Path

from projects.AcuRev100.tests.helpers_accuracy import run_accuracy_case

PROJECT_ROOT = Path(__file__).resolve().parents[2]      # <模块目录> -> tests -> AcuRev100
TEST_CONFIG = str(PROJECT_ROOT / "config.yaml")


def test_004_01_case3():
    report = run_accuracy_case(case_meta={
        '编号': 'Function_AC meter_004_01_case3',
        '标题': 'CT选型333mV(200A)，一路电流为（100A），在上位机均配置为对应A相，测量Ia,Ib,Ic,Iavg满足精度0.2%',
        '预置条件': '1、Acuview2上位机\n2、RTU串口线\n3、AcuRev-100电表\n4、接线方式3E4WY',
        '测试步骤': '1、连接AcuRev-100电表到Acuview2上，链接状态为connected\n2、输入一路电流分别为（2.5A），在上位机均配置为对应A相\n3、Acuview2上检查交流电相电流是否均在精度范围内',
        '预期结果': '3、Acuview2上检查交流电相电流均在精度范围内\nIa：99.800~100.200\nIb: 0~0\nIc: 0~0\nIavg: 33.267~33.400',
    }, config_path=TEST_CONFIG)
    assert report.passed
