...
路径约定：
  Excel 文件默认位于 <repo_root>/knowledge/shared/templates/raw/。
...
# ── 路径配置 ─────────────────────────────────────────────────────────────────
# 默认：本文件在 projects/AcuHMI_1_7/utils/，向上四级到仓库根，再进入 knowledge/shared/templates/raw/
_DEFAULT_DATA_MAP_DIR = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "knowledge" / "shared" / "templates" / "raw"
)
...
def load_device_params(device_name: str, snmp_only: bool = True, mqtt_only: bool = False) -> dict:
    """
    ...
    参数：
        ...
        mqtt_only:   True 时只返回 MQTT 列非空的行（即通过 MQTT 暴露的参数）
    ...
    """
    key = (device_name, snmp_only, mqtt_only)
    if key in _cache:
        return _cache[key]
    ...
    for row in rows[header_idx + 1:]:
        ...
        # MQTT 过滤
        if mqtt_only and col_mqtt is not None:
            mqtt_val = row[col_mqtt]
            if not mqtt_val or str(mqtt_val).strip() in ("", "None"):
                continue
    ...
    filter_label = "SNMP" if snmp_only else ("MQTT" if mqtt_only else "全量")
    log.info("[ExcelMap] %s(%s): 加载 %d 个参数，跳过 scale 缺失 %d 个",
             device_name, filter_label, len(result), skipped_no_scale)
    _cache[key] = result
    return result
