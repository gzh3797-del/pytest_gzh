...
# 用例编号: TestCase_WEB2_AZU_007_001
# 用例标题: 禁用 Azure IoT 后停止发布重新启用后恢复
# 预置条件: 设备已正常上电，相关服务正常启动
# 测试步骤:
#   1. 设备已连接 Azure IoT Hub，正常上报数据
#   2. 将 Enable 切换为 Disable，保存
#   3. 重新将 Enable 切换为 Enable，保存
# 预期结果: 2. 保存后不再向 Azure IoT Hub 发布任何数据 | 3. 重新启用后，Azure IoT Hub 正常收到设备上报数据
...
def test_TestCase_WEB2_AZU_007_001(login_page: LoginPage):
    login_page.open()
    login_page.login()
