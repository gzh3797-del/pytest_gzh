...
# 用例编号: TestCase_WEB2_AWS_004_001
# 用例标题: 4100WEB2选择4100 设备连接 AWS IoT Core 数据上报
# 预置条件: 1. 设备已正常上电，相关服务正常启动 | 2. 进入 AWS IoT 连接配置页面，启用 Enable | 3. 已配置正确的URL/Topic/Interval/证书文件
# 测试步骤:
#   1. Devices Selection To Mapping 勾选下挂的AcuRev4100设备
#   2. MQTT Parameter Config页面Parameter Type 选择 AI，Parameter 选择 ALL
#   3. 连接AWS IoT
#   4. 使用 MQTT 客户端监控设备向云端上报的参数是否为配置的4100设备参数
# 预期结果: 3. AWS IoT Core 收到正确的设备连接信息 | 4. MQTT客户端工具监控到设备向云端上报的数据为配置的数据，参数个数及参数值和采集的值一致
...
def test_TestCase_WEB2_AWS_004_001(login_page: LoginPage):
    login_page.open()
    login_page.login()
