...
# 用例编号: TestCase_AcuRev4100_WEB2_008_004_case30
# 用例标题: mqtt模块，检查字段"Devcies Selection"正确修改为"Devices Selection To Mapping红色星号"
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3.MQTT服务端启动正常
# 测试步骤:
#   1、mqtt页面，检查Devices Selection To Mapping红色星号
# 预期结果: 1、Devices Selection To Mapping后面加红色星号
...
def test_TestCase_AcuRev4100_WEB2_008_004_case30(login_page: LoginPage):
    ...
    expect(page.get_by_text("Devices Selection", exact=False).first).to_be_visible(),         "'Devices Selection' 字段应在页面可见"
