...
# 用例编号: TestCase_AcuRev4100_WEB2_008_003_case22
# 用例标题: Report Buffer Size参数设置验证
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3、NMS管理端已开启 | 4、SNMP已开启
# 测试步骤:
#   1.开启TRAP，并成功与NMS端建立连接
#   2.设置Report Buffer Size为0
#   3.设置Report Buffer Size为30
#   4.设置Report Buffer Size为31
...
def test_TestCase_AcuRev4100_WEB2_008_003_case22(login_page: LoginPage):
    ...
    buf_field = page.locator(".el-form-item").filter(has_text="Report Buffer Size").locator("input")
    if buf_field.count() == 0:
        pytest.skip("Report Buffer Size 字段未找到，可能 Trap Enable 未显示该字段")

    # 合法值 — 只检查 Report Buffer Size 自身的字段错误
    buf_item = page.locator(".el-form-item").filter(has_text="Report Buffer Size")
    for valid in ["5", "30"]:
        buf_field.fill(valid)
        page.get_by_role("button", name="Save").click()
        page.wait_for_timeout(500)
        assert buf_item.locator(".el-form-item__error").count() == 0, \
            f"Report Buffer Size={valid} 应保存成功"

    # 非法值 — 检查该字段或全局错误
    for invalid in ["31", "-1", "abc"]:
        buf_field.fill(invalid)
        page.get_by_role("button", name="Save").click()
        page.wait_for_timeout(500)
        assert buf_item.locator(".el-form-item__error").count() > 0 or \
