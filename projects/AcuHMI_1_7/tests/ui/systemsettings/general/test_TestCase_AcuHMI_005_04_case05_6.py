# 用例编号: TestCase_AcuHMI_005_04_case05_6
# 用例标题: 用户名为纯字母(Abcdefg)，保存成功
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 填写基准配置，将 Username 改为 Abcdefg（纯字母）
#   3. 点击 Save
# 预期结果: 保存成功
...
def test_TestCase_AcuHMI_005_04_case05_6(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    page.get_by_label("Username", exact=True).fill("Abcdefg")
