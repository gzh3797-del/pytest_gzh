# 用例编号: TestCase_AcuHMI_005_04_case06_2
# 用例标题: 密码为空，保存失败
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 填写基准配置，将 Password 清空
#   3. 点击 Save
# 预期结果: 保存失败，显示表单校验错误（密码不能为空）
...
@pytest.mark.xfail(strict=False, reason="System saves Email config with empty password (no validation enforced for empty password)")
def test_TestCase_AcuHMI_005_04_case06_2(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    page.get_by_label("Password", exact=True).clear()
