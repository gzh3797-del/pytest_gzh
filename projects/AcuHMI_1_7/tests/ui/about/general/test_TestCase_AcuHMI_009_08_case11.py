...
# 用例编号：TestCase_AcuHMI_009_08_case11
# 用例标题：Description字段输入恰好40个字符（上边界值），保存成功
# 预置条件：1.管理权限登录AcuHMI网页
# 测试步骤：
#   1. 进入About页面
#   2. 在Description字段输入恰好40个字符
#   3. 点击Save按钮
# 预期结果：
#   1. 保存成功，恰好等于最大长度40字符时允许保存
def test_TestCase_AcuHMI_009_08_case11(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    target = _DESCRIPTION_40 if desc_field.input_value() != _DESCRIPTION_40 else "z" * 40

    # 输入恰好40个字符的Description
    desc_field.clear()
    ...
    assert page.locator(".el-form-item__error").count() == 0, \
        "Description输入恰好40字符时页面不应显示验证错误"
