...
# 用例编号：TestCase_AcuHMI_009_07_case02_2
# 用例标题：选择间隔时间为1月，日志等级为错误，搜索事件日志并导出，
#           日志显示事件戳、等级及消息显示准确，并重置筛选条件
# 预置条件：1、管理权限登录AcuHMI网页
# 测试步骤：
#   1. Maintenance->Event Log设置间隔时间为1月，日志等级为Error
#   2. 点击Search，校验筛选结果（逐页校验Level和Timestamp）
#   3. 点击Reset，校验筛选条件清空
#   4. 无筛选Search，收集全量页面数据，导出日志Export Logs，比对内容是否一致
# 预期结果：
#   1. 设置成功
#   2. 筛选结果 Level 全部为 Error，Timestamp 在日期范围内
#   3. 筛选条件被重置
#   4. 导出文件行数与无筛选页面显示总行数一致，内容匹配
def test_TestCase_AcuHMI_009_07_case02_2(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    # Step 1: 设置 Interval=1月，Level=Error
    ...
    page.wait_for_timeout(200)
    page.get_by_role("option", name="Error").click()
    page.wait_for_timeout(200)
    ...
    filtered_total, _ = _collect_and_verify_all_pages(
        page, start_dt, end_dt, "Error", start_date, end_date
    )
    if filtered_total == 0:
        warnings.warn(
            f"筛选结果为0条Error日志（{start_date} ~ {end_date}），"
            "请确认该时间段内是否存在Error级别日志。",
