...
# 用例编号：TestCase_AcuHMI_007_03_case06_3
# 用例标题：设置宽限期为 65535（最大值），保存配置成功
# 测试步骤：
#   1. Grace Period = 65535
#   2. 点击 Save
# 预期结果：
#   保存成功（65535 为有效值）
def test_TestCase_AcuHMI_007_03_case06_3(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        assert not has_field_error and not has_error_toast, \
            "Grace Period=65535 是有效最大值，应保存成功"
    finally:
        _restore_grace(page)
