...
_ROLE_NAME = "pp01_9role"
_USER_NAME = "pp01_9user"
# 密码含数字+字母但无特殊字符，在 Numbers+Letters+Special 策略下应拒绝
_BAD_PWD   = "12345abc"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_9
# 用例标题：配置密码策略，勾选数字和字母、特殊字符，创建用户密码包数字和字母，无法创建用户
# 测试步骤：
#   1. Password Policy 勾选 Numbers+Letters 和 Special Characters，保存
#   2. 创建用户，密码为 "12345abc"（含数字+字母，无特殊字符）
# 预期结果：
#   2. 无法创建用户
def test_TestCase_AcuHMI_007_03_case01_9(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    _create_role(page)
    try:
        # Step 1: 启用 Numbers+Letters 和 Special Characters
        _set_policy(page, upper_lower=False, numbers_letters=True, special_chars=True)

        # Step 2: 创建用户，密码含数字+字母但无特殊字符 → 应失败
        _nav_to_submenu(page, "User Configuration")
        ...
        assert page.get_by_label("Password", exact=True).is_visible(), \
            "Numbers+Letters+Special 策略下，无特殊字符的密码创建用户应失败（对话框应保持打开）"
