...
_ROLE_NAME = "pp01_13role"
_USER_NAME = "pp01_13user"
# 密码含大写+小写+数字但无特殊字符，在三重策略下应拒绝
_BAD_PWD   = "Abc12345"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_13
# 用例标题：配置密码策略，勾选所有三项，创建用户密码包数字、大写和小写字母，无法创建用户
# 测试步骤：
#   1. Password Policy 勾选所有三项，保存
#   2. 创建用户，密码为 "Abc12345"（含大写+小写+数字，无特殊字符）
# 预期结果：
#   2. 无法创建用户
def test_TestCase_AcuHMI_007_03_case01_13(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        # Step 2: 创建用户，密码含大写+小写+数字但无特殊字符 → 应失败
        _nav_to_submenu(page, "User Configuration")
        ...
        assert page.get_by_label("Password", exact=True).is_visible(), \
            "三重策略下，无特殊字符的密码创建用户应失败（对话框应保持打开）"
