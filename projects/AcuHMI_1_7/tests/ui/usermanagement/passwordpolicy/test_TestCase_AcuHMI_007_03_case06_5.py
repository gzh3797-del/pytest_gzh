...
# 用例编号：TestCase_AcuHMI_007_03_case06_5
# 用例标题：设置宽限期为小数、字母、特殊字符，保存配置失败，提示错误信息正确
# 测试步骤：
#   依次将 Grace Period 设置为 "1.5"、"abc"、"@#"，点击 Save
# 预期结果：
#   各次保存均失败，提示错误信息正确
def test_TestCase_AcuHMI_007_03_case06_5(login_page: LoginPage):
    login_page.open()
    login_page.login()
