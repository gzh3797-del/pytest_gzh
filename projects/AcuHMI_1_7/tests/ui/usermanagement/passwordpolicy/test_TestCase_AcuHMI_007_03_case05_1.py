...
_ROLE_NAME = "pp05_1role"
_USER_NAME = "pp05_1user"
_SHORT_PWD = "Ac@1"          # 4 chars — satisfies content policy but < 8 (actual min)
_GOOD_PWD  = "Ac@12345"      # 8 chars — satisfies all policies (actual minimum valid)
...
def _try_create_user(page, username, password):
    """Attempt to create user; return True if user row appears in table."""
...
# 用例编号：TestCase_AcuHMI_007_03_case05_1
# 用例标题：设置最小密码长度为 6，密码长度不足 6 时新增用户保存失败，满足长度时保存成功，且该用户可登录
# 测试步骤：
#   1. Minimum Password Length = 6，保存
#   2. 新增用户，Password 长度 < 6 → 保存失败
#   3. 新增用户，Password 长度 = 6 → 保存成功
#   4. 使用新用户登录
# 预期结果：
#   2. 保存失败
#   3. 保存成功
#   4. 登录成功
def test_TestCase_AcuHMI_007_03_case05_1(login_page: LoginPage):
    login_page.open()
    login_page.login()
