...
_ROLE_NAME  = "pp01_1role"
_USER_NAME  = "pp01_1user"
_INIT_PWD   = "Admin@110001"
# 密码无大小写字母（仅特殊字符+数字），在 Upper+Lower 策略下应拒绝
_BAD_PWD    = "####12345"
...
# Password Policy checkbox indices (order on page)
_IDX_UPPER_LOWER   = 0
...
# 用例编号：TestCase_AcuHMI_007_03_case01_1
# 用例标题：配置密码策略，仅勾选大写和小写，创建用户密码不包含大小写，无法创建用户
# 测试步骤：
#   1. Password Policy 仅勾选 Upper and Lower Case，保存
#   2. 创建用户，密码为 "####12345"（无大小写字母）
# 预期结果：
#   1. 保存成功
#   2. 创建用户失败
def test_TestCase_AcuHMI_007_03_case01_1(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        assert page.get_by_label("Password", exact=True).is_visible(), \
            "Upper+Lower 策略下，无大小写的密码创建用户应失败（对话框应保持打开）"
