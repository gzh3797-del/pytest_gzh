...
# 用例编号：TestCase_AcuHMI_007_03_case05_4
# 用例标题：设置最小密码长度为 65，保存配置失败，提示错误信息正确
# 测试步骤：
#   1. Minimum Password Length = 65（超出有效最大值 64）
#   2. 点击 Save
# 预期结果：
#   保存失败，提示错误信息正确
def test_TestCase_AcuHMI_007_03_case05_4(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        assert (has_field_error or has_error_toast or not success_visible), \
            "Minimum Password Length=65 超出有效范围（6-64），应保存失败"
    finally:
