...
_ROLE_NAME  = "pp07_2role"
_USER_NAME  = "pp07_2user"
...
# 用例编号：TestCase_AcuHMI_007_03_case07_2
# 用例标题：设置最大失败尝试次数为 29，登录尝试 29 次失败后，显示等待 5 秒后再尝试
# 测试步骤：
#   1. Maximum Failed Attempts=29, Window=30, Wait=5，保存
#   2. 创建测试用户
#   3. 使用错误密码登录 29 次
#   4. 验证登录被锁定（正确密码也失败）
#   5. 等待 6 秒后，正确密码可登录
...
#   3. 29 次失败后账户被锁定
#   5. 解锁后登录成功
def test_TestCase_AcuHMI_007_03_case07_2(login_page: LoginPage):
    login_page.open()
    login_page.login()
