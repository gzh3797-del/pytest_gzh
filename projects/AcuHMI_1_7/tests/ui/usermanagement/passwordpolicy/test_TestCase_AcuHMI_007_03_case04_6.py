...
def _restore_expires(page):
    _nav_to_submenu(page, "Password Policy")
    page.get_by_placeholder("Enter Password Expires").fill("")
...
def _set_and_verify_fail(page, value: str):
    _nav_to_submenu(page, "Password Policy")
    inp = page.get_by_placeholder("Enter Password Expires")
    inp.fill(value)
    page.get_by_role("button", name="Save").click()
    page.wait_for_timeout(1000)
    ...
    assert (has_field_error or has_error_toast or not success_visible), \
        f"Password Expires={value!r} 应保存失败"


# 用例编号：TestCase_AcuHMI_007_03_case04_6
# 用例标题：设置密码过期为小数、特殊字符、字母，保存配置失败，提示错误信息正确
# 测试步骤：
#   依次将 Password Expires 设置为 "1.5"、"@#"、"abc"，点击 Save
# 预期结果：
#   各次保存均失败，提示错误信息正确
def test_TestCase_AcuHMI_007_03_case04_6(login_page: LoginPage):
    login_page.open()
    login_page.login()
