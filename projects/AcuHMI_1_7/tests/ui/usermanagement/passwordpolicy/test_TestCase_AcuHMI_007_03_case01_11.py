...
_ROLE_NAME = "pp01_11role"
_USER_NAME = "pp01_11user"
# 密码含大写+小写+数字+特殊字符（满足三重策略）
_GOOD_PWD  = "Abc123@#"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_11
# 用例标题：配置密码策略，勾选所有三项，创建用户密码包数字、特殊字符、大写和小写字母，可登录成功
# 测试步骤：
#   1. Password Policy 勾选所有三项（Upper+Lower, Numbers+Letters, Special Characters），保存
#   2. 创建用户，密码为 "Abc123@#"（含大写+小写+数字+特殊字符）
#   3. 新用户登录系统
# 预期结果：
#   1. 提示 "Password policy configuration saved"
#   2. 添加成功
#   3. 登录成功
def test_TestCase_AcuHMI_007_03_case01_11(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        # Step 1: 启用三重策略
        _set_policy(page, upper_lower=True, numbers_letters=True, special_chars=True)

        # Step 2: 创建用户，密码含大写+小写+数字+特殊字符 → 应成功
        _nav_to_submenu(page, "User Configuration")
        ...
        assert not page.get_by_label("Password", exact=True).is_visible(), \
            "三重策略下，含大写+小写+数字+特殊字符的密码创建用户应成功（对话框应关闭）"
