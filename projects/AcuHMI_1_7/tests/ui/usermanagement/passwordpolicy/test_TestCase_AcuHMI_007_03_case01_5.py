...
_ROLE_NAME = "pp01_5role"
_USER_NAME = "pp01_5user"
_INIT_PWD  = "Admin@110001"
# 密码无特殊字符，在 Special Characters 策略下应拒绝
_BAD_PWD   = "Abcd1234"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_5
# 用例标题：配置密码策略，仅勾选特殊字符，创建用户密码不包含特殊字符，无法创建用户
# 测试步骤：
#   1. Password Policy 仅勾选 Special Characters，保存
#   2. 创建用户，密码为 "Abcd1234"（无特殊字符）
# 预期结果：
#   2. 无法创建用户
def test_TestCase_AcuHMI_007_03_case01_5(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    _create_role(page)
    try:
        # Step 1: 仅启用 Special Characters 策略
        _set_policy(page, upper_lower=False, numbers_letters=False, special_chars=True)

        # Step 2: 创建用户，密码无特殊字符 → 应失败
        _nav_to_submenu(page, "User Configuration")
        ...
        assert page.get_by_label("Password", exact=True).is_visible(), \
            "Special Characters 策略下，无特殊字符的密码创建用户应失败（对话框应保持打开）"
