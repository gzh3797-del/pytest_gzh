...
_ROLE_NAME = "pp01_12role"
_USER_NAME = "pp01_12user"
# 密码含大写+小写+特殊字符但无数字，在三重策略下应拒绝
_BAD_PWD   = "Abc@XYZ!"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_12
# 用例标题：配置密码策略，勾选所有三项，创建用户密码包特殊字符、大写和小写字母，无法创建用户
# 测试步骤：
#   1. Password Policy 勾选所有三项，保存
#   2. 创建用户，密码为 "Abc@XYZ!"（含大写+小写+特殊字符，无数字）
# 预期结果：
#   2. 无法创建用户
def test_TestCase_AcuHMI_007_03_case01_12(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        # Step 2: 创建用户，密码含大写+小写+特殊字符但无数字 → 应失败
        _nav_to_submenu(page, "User Configuration")
