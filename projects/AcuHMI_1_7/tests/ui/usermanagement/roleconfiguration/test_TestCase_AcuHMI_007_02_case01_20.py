...
_ROLE_NAME = "rc01_20usre"
_USER_NAME = "rcuser01_20"
_INIT_PWD  = "Admin@110001"

# User=edit, all others=none
_PERM_MAP = {
    "User":             "edit",
    "Device":           "none",
    "Data Log":         "none",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_20
# 用例标题：添加角色，user权限为编辑，其余均为无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration -> Add Role，User=edit，其余均为 none
#   2. User Configuration -> Add User（user18 / role18），角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   3. 落地于 User Management 页面，有 Add User 按钮（edit 权限），页面无 Tip 提示
def test_TestCase_AcuHMI_007_02_case01_20(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            assert "/#/login" not in p.url, \
                f"User=edit 角色用户登录应成功，当前 URL: {p.url}"
            assert "userManagement" in p.url, \
                f"User=edit 角色用户应落地于 User Management 页，当前 URL: {p.url}"
            ...
            # edit 权限：有 Add User 按钮
            expect(p.get_by_role("button", name="Add User")).to_be_visible(timeout=5000)
        finally:
            ctx.close()
    finally:
