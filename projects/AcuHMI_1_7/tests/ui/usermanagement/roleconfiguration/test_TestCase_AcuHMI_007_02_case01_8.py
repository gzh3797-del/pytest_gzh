...
_ROLE_NAME = "rc01_8dle"
_USER_NAME = "rcuser01_8"
_INIT_PWD  = "Admin@110001"

# Data Log=edit, all others=none
_PERM_MAP = {
    "User":             "none",
    "Device":           "none",
    "Data Log":         "edit",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_8
# 用例标题：添加角色，角色权限为数据记录-编辑，其余均为无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration -> Add Role，Data Log=edit，其余均为 none
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   3. 该用户权限为数据记录-编辑（落地页为 Data Log 页面，有 Save 按钮）
def test_TestCase_AcuHMI_007_02_case01_8(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            # 登录成功，落地页为 Data Log 页面
            assert "/#/login" not in p.url, \
                f"Data Log=edit 角色用户登录应成功，当前 URL: {p.url}"
            assert "dataLog" in p.url, \
                f"Data Log=edit 角色用户应落地于 Data Log 页面，当前 URL: {p.url}"
