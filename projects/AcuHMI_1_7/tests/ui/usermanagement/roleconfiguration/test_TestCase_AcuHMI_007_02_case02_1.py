...
_ROLE_NAME = "rc02_1none2edit"
_USER_NAME = "rcuser02_1"
...
# 用例编号：TestCase_AcuHMI_007_02_case02_1
# 用例标题：编辑角色，由角色权限全无改为全编辑，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration 创建全无权限角色并绑定用户
#   2. 编辑该角色，将所有权限改为 edit
#   3. test 用户登录系统
# 预期结果：
#   3. 登录成功，系统权限为编辑（User Configuration 有 Add User 按钮）
def test_TestCase_AcuHMI_007_02_case02_1(login_page: LoginPage):
    login_page.open()
    login_page.login()
    admin_page = login_page.page
    browser = admin_page.context.browser

    _create_role(admin_page, _ROLE_NAME, "none")
    _create_user(admin_page, _USER_NAME, _INIT_PWD, role=_ROLE_NAME)
    try:
        _edit_role(admin_page, _ROLE_NAME, "edit")

        ctx, p = _login_as_user(browser, _USER_NAME, _INIT_PWD)
        try:
            assert "/#/login" not in p.url, \
                f"全编辑角色用户登录应成功，当前 URL: {p.url}"
