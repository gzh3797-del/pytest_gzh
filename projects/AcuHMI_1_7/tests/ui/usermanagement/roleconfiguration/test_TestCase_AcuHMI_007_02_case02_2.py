...
_ROLE_NAME = "rc02_2view2edit"
_USER_NAME = "rcuser02_2"
...
# 用例编号：TestCase_AcuHMI_007_02_case02_2
# 用例标题：编辑角色，由角色权限全视图改为全编辑，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. test2 用户角色权限为全视图，登录该用户查看权限
#   2. 修改该用户角色权限为全编辑，登录该用户查看权限
# 预期结果：
#   1. 该用户权限均为只读（User Configuration 无 Add User 按钮）
#   2. 该用户权限均为编辑（User Configuration 有 Add User 按钮）
def test_TestCase_AcuHMI_007_02_case02_2(login_page: LoginPage):
    login_page.open()
    login_page.login()
    admin_page = login_page.page
    browser = admin_page.context.browser

    _create_role(admin_page, _ROLE_NAME, "view")
    _create_user(admin_page, _USER_NAME, _INIT_PWD, role=_ROLE_NAME)
    try:
        # Step 1: 全视图 → User Config 无 Add User
        ctx1, p1 = _login_as_user(browser, _USER_NAME, _INIT_PWD)
        try:
            assert "/#/login" not in p1.url, \
                f"全视图角色用户登录应成功，当前 URL: {p1.url}"
            _nav_to_submenu(p1, "User Configuration")
            assert not p1.get_by_role("button", name="Add User").is_visible(), \
                "全视图角色用户在 User Configuration 中不应有 Add User 按钮"
        finally:
            ctx1.close()

        # 编辑角色为全编辑
        _edit_role(admin_page, _ROLE_NAME, "edit")

        # Step 2: 全编辑 → User Config 有 Add User
        ctx2, p2 = _login_as_user(browser, _USER_NAME, _INIT_PWD)
        try:
            assert "/#/login" not in p2.url, \
                f"全编辑角色用户登录应成功，当前 URL: {p2.url}"
            _nav_to_submenu(p2, "User Configuration")
            expect(p2.get_by_role("button", name="Add User")).to_be_visible(timeout=5000)
        finally:
            ctx2.close()
    finally:
