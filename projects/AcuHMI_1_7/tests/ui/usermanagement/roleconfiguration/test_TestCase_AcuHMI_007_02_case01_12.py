...
_ROLE_NAME = "rc01_12pe"
_USER_NAME = "rcuser01_12"
_INIT_PWD  = "Admin@110001"

# Protocol=edit, all others=none
_PERM_MAP = {
    "User":             "none",
    "Device":           "none",
    "Data Log":         "none",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_12
# 用例标题：添加角色，角色权限为协议-编辑，其余均为无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration -> Add Role，Protocol=edit，其余均为 none
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   3. 该用户权限为协议-编辑（落地页为 Protocol 页，有 Save 按钮）
def test_TestCase_AcuHMI_007_02_case01_12(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            assert "/#/login" not in p.url, \
                f"Protocol=edit 角色用户登录应成功，当前 URL: {p.url}"
            assert "protocols" in p.url, \
                f"Protocol=edit 角色用户应落地于 Protocol 页，当前 URL: {p.url}"
