...
_ROLE_NAME = "rc02_3view2none"
_USER_NAME = "rcuser02_3"
...
# 用例编号：TestCase_AcuHMI_007_02_case02_3
# 用例标题：编辑角色，由角色权限全视图改为全无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. test2 用户角色权限为全视图，登录该用户查看权限
#   2. 修改该用户角色权限为全无，登录该用户查看权限
# 预期结果：
#   1. 该用户权限均为只读（登录成功）
#   2. 该用户权限均为无（登录时提示无权限，停留在登录页）
def test_TestCase_AcuHMI_007_02_case02_3(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        # 编辑角色为全无
        _edit_role(admin_page, _ROLE_NAME, "none")

        # Step 2: 全无 → 提示无权限，停留在登录页
        ctx2, p2 = _login_as_user(browser, _USER_NAME, _INIT_PWD)
        try:
            expect(p2.get_by_text("no page access permissions", exact=False)).to_be_visible(timeout=5000)
            assert "/#/login" in p2.url, \
                f"全无权限角色用户应停留在登录页，当前 URL: {p2.url}"
        finally:
            ctx2.close()
    finally:
