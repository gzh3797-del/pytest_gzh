...
_ROLE_NAME = "rc01_5dv"
_USER_NAME = "rcuser01_5"
_INIT_PWD  = "Admin@110001"

# Device=view, all others=none
_PERM_MAP = {
    "User":             "none",
    "Device":           "view",
    "Data Log":         "none",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_5
# 用例标题：添加角色，角色权限为装置-视图，其余均为无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration -> Add Role，Device=view，其余均为 none
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   3. 该用户权限为装置-视图（落地页为 Device Dashboard，Physical Devices 页无 Add Device 按钮）
def test_TestCase_AcuHMI_007_02_case01_5(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            assert "/#/login" not in p.url, \
                f"Device=view 角色用户登录应成功，当前 URL: {p.url}"
            assert "dashboard" in p.url, \
                f"Device=view 角色用户应落地于 Device Dashboard，当前 URL: {p.url}"

            # 导航到 Physical Devices（Device 区域侧边栏，用 text 点击）
            p.get_by_text("Physical Devices", exact=True).first.click()
            p.wait_for_load_state("networkidle")
            p.wait_for_timeout(500)

            # view 权限：Physical Devices 页无 Add Device 按钮
            add_btn = p.get_by_role("button", name="Add Device")
            assert not add_btn.is_visible(), \
                "Device=view 角色用户在 Physical Devices 页不应有 Add Device 按钮"
        finally:
            ctx.close()
    finally:
