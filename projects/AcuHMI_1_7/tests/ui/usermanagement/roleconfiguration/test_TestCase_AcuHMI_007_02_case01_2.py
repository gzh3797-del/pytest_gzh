...
_ROLE_NAME = "rc01_2edit"
_USER_NAME = "rcuser01_2"
...
def _create_role_all_edit(page, role_name: str):
...
# 用例编号：TestCase_AcuHMI_007_02_case01_2
# 用例标题：添加角色，角色权限均为编辑，创建该用户，登录后查看该用户权限
# 预置条件：管理权限登录AcuHMI网页
# 测试步骤：
#   1. Role Configuration -> Add Role，所有权限设置 edit，点击 Save
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   1. 保存成功
#   2. 创建成功
#   3. 登录成功，系统权限为编辑（User Config 中有 Add User 按钮）
def test_TestCase_AcuHMI_007_02_case01_2(login_page: LoginPage):
    login_page.open()
    login_page.login()
    admin_page = login_page.page
    browser = admin_page.context.browser

    _create_role_all_edit(admin_page, _ROLE_NAME)
    ...
    try:
        ...
        try:
            ...
            assert "/#/login" not in p.url, \
                f"all-edit 角色用户登录应成功，当前 URL: {p.url}"

            # 导航到 User Configuration，edit 角色应有 Add User 按钮
