...
# 用例编号：TestCase_AcuHMI_007_01_case03
# 用例标题：删除1、2个用户，登录所有用户
# 测试步骤：
#   1. 创建3个用户（del1、del2、del3）
#   2. 删除 del1 和 del2
#   3. 用 del1 和 del2 登录 → 应失败
#   4. 用 del3 登录 → 应成功
# 预期结果：
#   删除的用户无法登录，未删除的用户可以登录
def test_TestCase_AcuHMI_007_01_case03(login_page: LoginPage):
    login_page.open()
    login_page.login()
