...
_RTU_DEVICE_NAME = "AutoTest_RTU_Delete_case11"
...
def _add_rtu_device(page, device_name: str):
    """Add an RTU device via the Add Device form."""
    ...
    # 4. Protocol: 点击 RTU（RTU 默认已选，但显式点击确保）
    page.locator(".el-radio__label").filter(has_text="RTU").click()
    page.wait_for_timeout(500)

    # 5. COM Port（RTU 选中后出现，选第一个可用选项）
    com_fi = page.locator(".el-form-item").filter(has_text="COM Port").first
    if com_fi.count() > 0:
        try:
            com_fi.wait_for(state="visible", timeout=3000)
            com_fi.locator(".el-select").first.click()
            page.wait_for_timeout(300)
            _click_visible_option(page, "")
        except Exception:
            pass
...
# 用例编号：TestCase_AcuHMI_001_01_case11
# 用例标题：设备手动删除RTU方式添加接入设备，删除成功
# 预置条件：
#   1. 接入设备支持 Modbus RTU
# 测试步骤：
#   1. 通过 RTU 方式添加设备（Device Name 唯一，无需物理设备在线）
#   2. 验证设备出现在 Physical Devices 列表
#   3. 删除该设备
#   4. 验证设备已从列表中消失
# 预期结果：
#   设备添加并删除成功，删除后列表中不再显示该设备
def test_TestCase_AcuHMI_001_01_case11(login_page: LoginPage):
    login_page.open()
    login_page.login()
    page = login_page.page

    # 清理上次残留（如有）
    _delete_device_by_name(page, _RTU_DEVICE_NAME)

    # Step 1: 添加 RTU 设备
    _add_rtu_device(page, _RTU_DEVICE_NAME)

    # Step 2: 验证设备出现在列表
    _nav_to_physical_devices(page)
    added_row = page.locator("tbody tr").filter(has_text=_RTU_DEVICE_NAME)
    assert added_row.count() > 0, \
        f"RTU 设备 '{_RTU_DEVICE_NAME}' 应出现在 Physical Devices 列表中，但未找到"
    ...
    # Step 4: 验证设备已删除
    _nav_to_physical_devices(page)
    remaining = page.locator("tbody tr").filter(has_text=_RTU_DEVICE_NAME)
    assert remaining.count() == 0, \
        f"RTU 设备 '{_RTU_DEVICE_NAME}' 删除后应从列表中消失，但仍然存在"
