...
# 用例编号：TestCase_AcuHMI_003_04_case16
# 用例标题：验证Rapid Logger的log File length、log interval逻辑关系
# 预置条件：同上
# 测试步骤：
#   各file length对应的可选interval验证（含秒级interval选项）
# 预期结果：
#   各file length对应的interval可选范围正确
#   例：file length=1 minute时，interval可选: 1 second~30 seconds,1 minute
#       file length=1 month时，interval可选: 1 hour,6 hours,12 hours,1 day,7 days,1 month
...
_RAPID_LINKAGE = {
    ...
    "7 days": [
        "15 minutes", "30 minutes", "1 hour", "6 hours", "12 hours", "1 day", "7 days",
    ],
    "1 month": [
        "1 hour", "6 hours", "12 hours", "1 day", "7 days", "1 month",
    ],
}


def _nav_to_rapid_logger(page):
    """Navigate to Data Log > Data Loggers > Rapid Logger."""
...
def test_TestCase_AcuHMI_003_04_case16(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    for file_length, expected_intervals in _RAPID_LINKAGE.items():
        ...
        for expected in expected_intervals:
            if expected not in actual_options:
                failures.append(
                    f"[RapidLogger] FileLength={file_length}: "
