...
# 用例编号：TestCase_AcuHMI_003_04_case21
# 用例标题：验证logger1的log File length、log interval逻辑关系
# 预置条件：同上
# 测试步骤：
...
# 预期结果：
#   1.  interval仅可选1 minute
#   2.  interval可选1 minute、5 minutes
#   ... (联动关系)
...
def _nav_to_data_loggers1(page):
    """Navigate to Data Log > Data Loggers 1."""
    if "/#/dataLog" not in page.url:
        ...
        # 点击 Data Log 左侧导航，默认进入 Data Loggers 1
        page.locator(".left-nav-item").filter(has_text="Data Log").click()
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(500)
    elif "dataLogger1" not in page.url:
        # 已在 dataLog 页但不是 logger1，通过 Tab 下拉导航
...
def test_TestCase_AcuHMI_003_04_case21(login_page: LoginPage):
    login_page.open()
    login_page.login()
