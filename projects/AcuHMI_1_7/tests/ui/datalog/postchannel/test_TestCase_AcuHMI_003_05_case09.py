...
# 下拉里 Post Channel 2 选项的真实文案（带空格）
_POST_CHANNEL_2 = "Post Channel 2"
...
def _find_post_channel2_class(page: Page):
    """读取当前展开的 Post Channel 下拉里 'Post Channel 2' 选项的 class；不存在返回 None。"""
    for item in page.locator(".el-select-dropdown__item").all():
        try:
            if item.is_visible() and item.inner_text().strip() == _POST_CHANNEL_2:
...
# 用例编号：TestCase_AcuHMI_003_05_case09
# 用例标题：Post Ch2 设置为 disable，Logger 数据记录 Post Channel 选项无法选中 Post Ch2
# 预置条件：
#   1. 服务启动正常，账号登录成功
# 测试步骤：
#   1. 进入 Post Channels > Post Channel 2，将开关置为 Disable 并保存
#   2. 进入 Data Loggers > Data Loggers 1，Enable Logger 1
#   3. 打开 Post Channel 下拉，检查 Post Channel 2 是否不可选
# 预期结果：
#   1. 保存成功
#   2. Logger 1 的 Post Channel 下拉中 Post Channel 2 选项不可选（带 disabled 类）
def test_TestCase_AcuHMI_003_05_case09(login_page: LoginPage):
    login_page.open()
    login_page.login()
    page = login_page.page

    # Step 1：进入 Post Channel 2，置为 Disable 并保存
    _nav_submenu(page, "Post Channels", "Post Channel 2")
    page.wait_for_timeout(500)
    ...
    assert page.locator(".el-message--error").count() == 0, \
        "Post Channel 2 置为 Disable 保存应成功，但出现了错误提示"
    ...
    # Step 3：打开 Post Channel 下拉，校验 Post Channel 2 不可选
    ...
    cls = _find_post_channel2_class(page)
    page.keyboard.press("Escape")
    page.wait_for_timeout(300)

    # 断言“必须找到”防止下拉未展开时的空转假通过；“带 disabled”捕获“仍可选”这一缺陷
    assert cls is not None, \
        "Post Channel 下拉中未找到 'Post Channel 2' 选项，无法确认其不可选"
    assert "disabled" in cls, \
        f"Post Channel 2 Disable 后，Logger 1 下拉中该项应不可选，但 class='{cls}'"
