...
# 用例标题：Post Ch2设置为disable，Logger数据记录PostChannel选项无法选中Post Ch2
# 预置条件：
#   1. 服务启动正常，账号登录成功
# 测试步骤：
#   1. 进入 Post Channels > Post Channel 2，将开关置为 Disable 并保存
#   2. 进入 Data Loggers > Data Loggers 1，Enable Logger 1
#   3. 打开 Post Channel 下拉，查看 Channel2 是否不可选（disabled 或不存在）
# 预期结果：
#   1. 保存成功
#   2. Logger 1 Post Channel 下拉中 Channel2 选项不可选或不存在


def _nav_to_post_channel2(page):
    """Navigate to Data Log > Post Channels > Post Channel 2."""
    ...
    # 点击 Post Channel 2
    item = page.locator(".el-menu-item").filter(has_text="Post Channel 2")
...
def test_TestCase_AcuHMI_003_05_case09(login_page: LoginPage):
    ...
    # Step 1: 进入 Post Channel 2，设为 Disable 并保存
    _nav_to_post_channel2(page)
    page.wait_for_timeout(500)
    ...
    assert page.locator(".el-message--error").count() == 0, \
        "Post Channel 2 Disable 保存应成功，但出现了错误提示"
    ...
    # Step 3: 打开 Post Channel 下拉，验证 Channel2 不可选
    ...
    for it in all_items:
        try:
            if it.is_visible() and "Channel2" in it.inner_text():
                channel2_item = it
                break
        except Exception:
            pass
    ...
    if channel2_item is None:
        # Channel2 不在选项列表中 — 符合预期（disabled 后被移除）
        pass
    else:
        cls = channel2_item.get_attribute("class") or ""
        assert "disabled" in cls, (
            "Post Channel 2 Disable后，Logger 1 的 Post Channel 下拉中 "
            f"Channel2 选项应不可选，但 class='{cls}'"
        )
