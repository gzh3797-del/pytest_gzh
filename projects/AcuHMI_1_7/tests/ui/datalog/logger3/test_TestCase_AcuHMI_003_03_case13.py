...
# 用例编号：TestCase_AcuHMI_003_03_case13
# 用例标题：验证logger3的log File length、log interval逻辑关系
# 预置条件：同上
# 测试步骤：
...
# 预期结果：
#   各file length对应的interval可选范围正确（联动关系同Logger1/Logger2）
...
def test_TestCase_AcuHMI_003_03_case13(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    for file_length, expected_intervals in _LOGGER_LINKAGE.items():
        ...
        for expected in expected_intervals:
            if expected not in actual_options:
                failures.append(
                    f"[Logger3] FileLength={file_length}: "
