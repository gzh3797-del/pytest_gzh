...
Playwright + pytest 的 Web UI 测试。**所有命令在仓库根目录 `autotest/` 下执行**（用例用绝对导入 `from projects.acuhmi_1_7...`，不能在子目录里跑），用 `pytest` 启动器（不要用 `pytest`，避免 IDE 解释器路径问题）。
...
部分用例会**重启被测设备**，单条耗时约 2-3 分钟、会中断设备服务。**运行前先确认设备可用、无人占用**。已知高危用例集中在系统设置（见下方该子模块小节）。重启类用例已带 `@pytest.mark.destructive`，可用 `-m "not destructive"` 排除。

## 子模块索引

| 子模块 | 目录 | 用例(文件)数 | 对应测试用例 / 编号 |
|--------|------|:-----------:|---------------------|
| 系统设置 | `systemsettings/` | 45 | 005_*（+Remote Access） |
...
pytest projects/AcuHMI_1_7/tests/ui/systemsettings/general -k "005_01" -v    # Date & Time               10/10
pytest projects/AcuHMI_1_7/tests/ui/systemsettings/general -k "005_02" -v    # Network                    2/8
pytest projects/AcuHMI_1_7/tests/ui/systemsettings/general -k "005_04" -v    # Email                     24/29
pytest projects/AcuHMI_1_7/tests/ui/systemsettings/general -k "005_05" -v    # Alarm Notification         6/11
...
# ⚠️ 高危（会重启设备，跑前确认）：005_01_case01（重启）
```
