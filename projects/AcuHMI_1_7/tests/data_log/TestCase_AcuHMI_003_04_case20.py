# -*- coding: utf-8 -*-
"""
用例 ID：TestCase_AcuHMI_003_04_case20
类别：F — 推送中断与恢复验证
功能：Logger1 推送 FTP/CSV 正常后，禁用 Logger 模拟中断，恢复后验证文件继续产生

【预置条件】
  1. setup_env.py 已在后台运行：FTP 服务器已启动，Post Channel 1=FTP 已配置并 Enabled
...
【测试步骤】
  ...
  2. 等待最长 450 秒，轮询 FTP 接收目录直至出现 .csv 文件（验证正常推送）
  3. 将 Logger1 设为 Disable（模拟网络中断/停止推送）
  4. 等待 90 秒，检查 FTP 目录文件数量不增加
  5. 重新启用 Logger1（恢复推送），等待新文件出现
  6. 验证恢复后 FTP 目录有新文件产生

【预期结果】
  2. FTP 目录在 450s 内收到文件（正常推送）
  3-4. Logger1 禁用期间 FTP 目录文件数不增加
  5-6. 重新启用后 FTP 目录出现新文件
"""
import os
import time
...
CASE_ID     = "TestCase_AcuHMI_003_04_case20"
LOGGER_N    = 1
PROTOCOL    = "FTP"
FILE_FORMAT = "csv"
