...
pytest projects/AcuHMI_1_7/tests/data_log/ -k "003_01" -v   # 全部 Logger1 用例
pytest projects/AcuHMI_1_7/tests/data_log/ -k "003_02" -v   # 全部 Logger2 用例
pytest projects/AcuHMI_1_7/tests/data_log/ -v               # 全部用例
pytest projects/AcuHMI_1_7/tests/data_log/TestCase_AcuHMI_003_01_case01.py -v   # 单条用例
```

> 用 `-k` 关键字过滤用例，不要用 `TestCase_*.py` 通配符（PowerShell 不展开 `*`）。
> 若之前跑过 `setup_env.py` 残留了 `.setup_done`，会让 conftest 跳过自动起服务器；
> 先 `python projects/AcuHMI_1_7/tests/data_log/teardown_env.py` 清掉再直跑。
...
```powershell
python projects/AcuHMI_1_7/tests/data_log/setup_env.py                 # 另开终端，阻塞运行，Ctrl+C 前保持
pytest projects/AcuHMI_1_7/tests/data_log/ -k "003_01" -v    # 可重复多次，秒级进入用例
python projects/AcuHMI_1_7/tests/data_log/teardown_env.py              # 停服务器、清 .setup_done
```
