# -*- coding: utf-8 -*-
"""
用例 ID：TestCase_AcuHMI_003_02_case12
类别：F — 推送中断与恢复验证
功能：Logger2 推送 HTTP/JSON 正常后，禁用 Logger 模拟中断，恢复后验证文件继续产生
...
【测试步骤】
  ...
  3. 将 Logger2 设为 Disable（模拟中断）
  4. 等待 90 秒，检查 HTTP 目录文件数量不增加
  5. 重新启用 Logger2（恢复推送），等待新文件出现
  6. 验证恢复后 HTTP 目录有新文件产生

【预期结果】
  2. HTTP 目录在 450s 内收到文件（正常推送）
  3-4. Logger2 禁用期间 HTTP 目录文件数不增加
...
CASE_ID     = "TestCase_AcuHMI_003_02_case12"
LOGGER_N    = 2
PROTOCOL    = "HTTP"
FILE_FORMAT = "json"
