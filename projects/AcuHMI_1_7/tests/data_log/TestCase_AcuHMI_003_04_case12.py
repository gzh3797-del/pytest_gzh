# -*- coding: utf-8 -*-
"""
用例 ID：TestCase_AcuHMI_003_04_case12
类别：C — 正向推送验证（JSON，2 seconds 间隔）
功能：Rapid Logger 以 2 秒间隔通过 SFTP 推送 JSON，验证文件内容及推送频率
...
【预置条件】
  ...
  4. 所有 Physical Devices 的 Poll Interval 须 ≤ 2s（本用例自动设置并还原）

【测试步骤】
  1. 将所有 Physical Devices 的 Poll Interval 改为 2s
  2. 配置 Rapid Logger：
       Enable=True  PostChannel=2(SFTP)  Format=JSON  Length=5 minute
       NameFormat=Time Interval Format  Prefix=meter1_RapidLogger  Interval=2 seconds
  3. 等待最长 450s，轮询 SFTP 目录出现 .json 文件
  4. 验证文件名格式、时间戳格式、推送间隔（±30%）、行数（约 150 行）
  5. 将 Rapid Logger 设为 Disable
  6. 恢复所有 Physical Devices 的 Poll Interval 为原始值

【预期结果】
  - SFTP 目录收到 .json 文件，相邻记录间隔约 2 s，行数约 150 行
"""
from helpers import run_rapid_push_case
from datalog_page import PhysicalDevicePollHelper

CASE_ID     = "TestCase_AcuHMI_003_04_case12"
PROTOCOL    = "SFTP"
FILE_FORMAT = "json"
...
def test_case(pool, driver):
    ...
    originals = {}
    try:
        originals = poll_helper.set_all(2)
