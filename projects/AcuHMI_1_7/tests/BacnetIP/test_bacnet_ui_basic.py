...
运行：
  pytest projects/AcuHMI_1_7/tests/BacnetIP/test_bacnet_ui_basic.py -v
"""
...
def _get_all_device_rows(page: Page) -> list[dict]:
    """
    从 BACnet/IP 页面的 Devices Selection To Mapping 表格读取**全部行**（不按勾选过滤）。
...
def _get_all_device_names(page: Page) -> list[str]:
    """
    返回 Devices Selection To Mapping 表格中所有设备的名称（含未勾选行）。
...
def _set_device_checked(page: Page, device_name: str, checked: bool) -> bool:
    """
    在 Devices Selection To Mapping 表格中，按 device_name 精确匹配设备全名
...
def _restore_device_selection(page: Page, checked_names: list[str]) -> None:
    """
    将 Devices Selection To Mapping 表格的勾选状态恢复为 ``checked_names`` 集合：
    名称在集合内的行勾上，不在的取消。
...
def _open_param_dialog(page: Page, device_name: str) -> bool:
    """
    在 Devices Selection To Mapping 表格中找到 device_name（全名精确匹配，
