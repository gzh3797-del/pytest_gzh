"""
FTS编号: FTS_AcuHMI_AWS_004_003
用例标题: AcuHMI-1-7选择Acuvim3设备连接 AWS IoT Core 数据上报（三段式比较）
用例级别: LV1

预置条件:
  - Acuvim3 设备已接入网关并在线，合法证书已配置，MQTT 订阅端就绪
...
class TestCase_AcuHMI_1_7_AWS_004_003:

    @pytest.mark.aws_iot
    @pytest.mark.slow
    def test_acuvim3_connect_and_verify(self, aws_page, aws_cfg):
        """配置 Acuvim3 → Test Connection → 三段式验证，一次浏览器完成"""
        aws = aws_cfg["aws_iot"]
