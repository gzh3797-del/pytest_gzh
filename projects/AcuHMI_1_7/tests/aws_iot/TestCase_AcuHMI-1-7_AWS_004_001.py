"""
FTS编号: FTS_AcuHMI_AWS_004_001
用例标题: AcuHMI-1-7选择AcuRev4100设备连接 AWS IoT Core 数据上报（三段式比较）
用例级别: LV1

预置条件:
  - AcuRev4100 设备已接入网关并在线，合法证书已配置，MQTT 订阅端就绪
...
EXPECTED_DEVICES = ["AcuRev4100"]


class TestCase_AcuHMI_1_7_AWS_004_001:

    @pytest.mark.aws_iot
    @pytest.mark.slow
    def test_acurev4100_connect_and_verify(self, aws_page, aws_cfg):
        """配置 AcuRev4100 → Test Connection → 三段式验证，一次浏览器完成"""
        aws = aws_cfg["aws_iot"]
