"""
FTS编号: FTS_AcuHMI_AWS_004_002
用例标题: AcuHMI-1-7选择AcuRev2100设备连接 AWS IoT Core 数据上报（三段式比较）
用例级别: LV1

预置条件:
  - AcuRev2100 设备已接入网关并在线，合法证书已配置，MQTT 订阅端就绪
...
class TestCase_AcuHMI_1_7_AWS_004_002:

    @pytest.mark.aws_iot
    @pytest.mark.slow
    def test_acurev2100_connect_and_verify(self, aws_page, aws_cfg):
        """配置 AcuRev2100 → Test Connection → 三段式验证，一次浏览器完成"""
        aws = aws_cfg["aws_iot"]
