"""
Testcase_AcuHMI_AcuRev4100_CurrSwl_Thr_003
原始编号: TC_CurrSwl_Thr_001 ~ TC_CurrSwl_Thr_005
"""
import sys, os
sys.path.insert(0, os.path.dirname(__file__))
from helpers_4100 import *  # noqa: F401, F403  # noqa: F401, F403


# ── 测试数据：value, ok ─────────────────────────────────────────
_CASES = [
    (110, True),                                              # TC_CurrSwl_Thr_001
    (130, True),                                              # TC_CurrSwl_Thr_002
    (150, True),                                              # TC_CurrSwl_Thr_003
    (109, False),                                             # TC_CurrSwl_Thr_004
    (151, False),                                             # TC_CurrSwl_Thr_005
]


def test_Testcase_AcuHMI_AcuRev4100_CurrSwl_Thr_003(app_page):
    """
    Testcase_AcuHMI_AcuRev4100_CurrSwl_Thr_003
    参数组: value, ok
    """
    for (value, ok) in _CASES:
        nav_to_event_waveform(app_page)
        set_pq_threshold(app_page, CSW_IDX, value)
        saved = save_and_check(app_page)
        _log.info("[TC] CurrSwellThreshold=%s  ok=%s  result=%s", value, ok, saved)
        if ok:
            assert saved, f"Current Swell Threshold={value}: 保存失败"
            verify_modbus(REG_CURR_SWL_THR, value, label=f"CurrSwellThreshold({value})")
        else:
            assert not saved, f"Current Swell Threshold={value}: 期望校验失败但保存成功"


