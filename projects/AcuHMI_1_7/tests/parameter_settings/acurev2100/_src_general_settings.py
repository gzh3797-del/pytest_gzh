...
2. Physical Devices -> 点击 AcuRev2100
...
def run_ui():
    with sync_playwright() as pw:
        ...
        try:
            ...
            page.get_by_text("Physical Devices", exact=True).first.wait_for(state="visible", timeout=15000)
            logging.info("[UI] 登录完成")

            # 2. Physical Devices
            logging.info("[UI] 进入 Physical Devices")
            page.get_by_text("Physical Devices", exact=True).first.click()
