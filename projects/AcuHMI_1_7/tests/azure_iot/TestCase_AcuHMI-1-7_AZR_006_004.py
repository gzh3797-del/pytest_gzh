"""
FTS编号: FTS_AcuHMI_AZR_006_004
用例标题: 被发布设备离线时上报行为与预期一致
用例级别: LV3

预置条件: 至少一台下游设备已在 Azure IoT 设备列表中，Azure IoT 正常配置并连接

测试步骤:
  1. 确认 Azure IoT 正常上报
  2. 将下游设备断电或拔线（使其离线）
  3. 等待 3 个 Interval，观察 Event Hub 消息（值为 null/0/错误码，网关不崩溃）
  4. 恢复下游设备，验证数据自动恢复

预期结果:
  - 设备离线时，Azure IoT Hub 消息行为符合规格，网关不崩溃
  - 下游设备恢复后，数据自动恢复
"""
...
class TestCase_AcuHMI_1_7_AZR_006_004:

    @pytest.mark.azure_iot
    @pytest.mark.slow
    @pytest.mark.manual
    def test_upstream_connected_before_device_offline(self, azure_page, azure_cfg):
        """步骤1：确认 Azure IoT 初始正常上报"""
    ...
    def test_behavior_when_device_offline(self, azure_cfg):
        """步骤2-4：下游设备离线期间验证 Event Hub 行为（网关不崩溃）"""
        time.sleep(100)
    ...
    def test_data_recovers_after_device_online(self, azure_cfg):
        ...
        assert ok, "下游设备恢复后 Azure IoT 数据应正常"
