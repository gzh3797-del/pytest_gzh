...
状态字符串与 WEB2 页面 Wiring Status 列文字一致。
"""
import cmath
import math
...
def compute_3e4wy(
    ...
    # ── Step 2：反接（无缺失时执行，按顺序，满足则停止）──────────────────────
        ...
        # 条件13：角度法检测相序错误（与 Delta 引擎一致）
        # ABC 配置：检测到 ACB 信号（∠VB≈120°且∠VC≈240°）→ Phase Order Error
        # ACB 配置：检测到 ABC 信号（∠VB≈240°且∠VC≈120°）→ Phase Order Error
    ...
    # ── 电流侧（各相独立，与电压侧无关）─────────────────────────────────────
    # 相电压为 0 时无有效相角参考，用 _current_status_v 处理该边界
