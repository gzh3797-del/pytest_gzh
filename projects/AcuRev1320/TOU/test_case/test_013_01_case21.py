...
"""
Function_AcuRev1320_013_01_case21
"""

dst_format = 2
(start_month, start_day, start_week, start_time, start_adjust_time) = (1, 1, 2, '02:00', 60)
(end_month, end_day, end_week, end_time, end_adjust_time) = (2, 4, 5, '03:00', 60)
(end_month2, end_day2, end_week2, end_time2, end_adjust_time2) = (7, 1, 2, '03:00', 60)
time_year, time_month, time_day, time_hour, time_minute, time_second = 2025, 2, 27, 2, 59, 55
...
def test_013_01_case21():
    ...
    # 上位机点击update，检查是否更新成功
    update_success = tou.click_update()
    logging.info(f'电表关闭DST_enable')
    assert update_success
    (year, month, day, hour, minute, second) = tou.check_device_time()
    # 检查电表时间是否增加1小时
    if hour == time_hour:
        logging.info(f'电表时间未跳变，时间保持')
        assert True
    else:
        assert False
