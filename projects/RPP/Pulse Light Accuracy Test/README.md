...
meter:                     # 被测电表 Modbus
  ...
  wiring:                  # 接线方式寄存器 + 映射（按电表手册改）
    register: 4162
    dtype: uint16
    map:                  # 工作表名 -> 写入寄存器的值（可增删改）
      "1E2W_脉冲灯测点": 0
      "2E3w1p_脉冲灯测点(optional)": 1
      "2E3W_Delta_脉冲灯测点": 2
      "2E3wN_脉冲灯测点(optional)": 3
      "3E4W Y_脉冲灯测点": 4
      "3E3wD接线_脉冲灯测点(optional)": 5
...
> 以 **AcuRev1320** 为例：脉冲常数寄存器 4122(uint32, ×1000)；接线方式寄存器 4162(uint16)，
> 值 `0=1E2W, 1=2元件三线phase1, 2=2元件三线Delta, 3=2元件三线Network, 4=3元件四线Y, 5=3元件四线Delta`。
