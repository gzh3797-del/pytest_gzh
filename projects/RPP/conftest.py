...
from projects.RPP.settings import HEADLESS, SLOW_MO, BASE_URL
...
# 注：.env 已由 projects.RPP.settings 适配层加载，无需在此重复 load_dotenv。
...
def login_page(page):
    ...
    from projects.RPP.pages.login_page import LoginPage  ##导入类LoginPage
...
def pytest_runtest_makereport(item, call):
    ...
    if rep.when == "call" and rep.failed:
        ...
        if page is not None:
            try:
                from projects.RPP.settings import get_screenshot_dir
                from projects.RPP.helpers.web_helpers import timestamp
                png = page.screenshot()
...
# RPP 目前没有 parameter_settings 子模块（此表原样照抄自 AcuHMI-1-7 的
# acurev4100/acuvim_iiw，与 RPP 无关，已清空），allure_labels fixture 在查不到
# 映射时会回退用 (device_dir, func) 作 feature/story，不影响现有 mqtt 用例。
