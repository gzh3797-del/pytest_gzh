...
from projects.RPP.tests.demand_test.memory_addrs import MemoryAddr, SlaveId, MemoryReg
...
class HandleMemory:
    ...
    def read_demand_in(self):
        ...
        RPP 无中线电流需量寄存器: demand_addr 缺 phase_n_current 键时返回 None,
        由调用方跳过 In 比对。
        :return:phase_n_current 或 None
        """
        address = MemoryAddr.demand_addr.get("phase_n_current")
        if address is None:
            self.log.info('RPP 无中线电流需量寄存器, read_demand_in 返回 None(跳过 In 比对)')
            return None
        count = MemoryReg.reg_float32
        slave = self.slave_id
        measure_value = self.modbus_client.read_measurement(address=address, count=count, device_id=slave)
    ...
    def set_sys_millisecond(self, sys_millisecond):
        """
        写系统时间毫秒寄存器(用于"时间触发"需量重置)。
        RPP 无该寄存器(时间走 NTP/PTP), 调用即报错。
        :param sys_millisecond:
        """
        address = MemoryAddr.sys_millisecond
        if address is None:
            raise RuntimeError(
                'RPP 不支持经 Modbus 写系统时间, demand_trigger=1(时间触发)不可用, '
                '请在用例数据中改用 demand_trigger=0(重设参数)或 2(清最大需量)'
            )
