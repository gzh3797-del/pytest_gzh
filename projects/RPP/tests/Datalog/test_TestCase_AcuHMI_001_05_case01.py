...
# 用例编号：TestCase_AcuHMI_001_05_case01
# 用例标题：接入设备 Datalog 栏，通过日志产生时间区间和间隔
#          （1mins、5mins、10mins、15mins、30mins），可下载日志到本地 PC，
#          导出日志格式为 .csv
# 预置条件：
#   1. AcuHMI 上电
#   2. 设备已接入并在线，且已采集数据超过 1 小时
# 测试步骤：
#   1-11. 选中有 datalog 的时间段，依次选择 1/5/10/15/30 mins 间隔，
#         点击导出并检查文件内容和格式
# 预期结果：各间隔导出文件数据时间戳及时间间隔正确
# 说明：实测导出产物为 gzip 压缩的 CSV（*.csv.gz），解压后校验；
#      时间区间使用页面默认区间（起止均为有数据日期，面板禁选无数据日）。

_INTERVALS = ("1 minute", "5 minutes", "10 minutes", "15 minutes",
              "30 minutes")


def test_TestCase_AcuHMI_001_05_case01(app_page: Page, tmp_path: Path):
    page = app_page
    ...
    # 设计约束：Log Interval 必须小于数据时长，只测数据跨度足够的档位
