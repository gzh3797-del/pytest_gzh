...
# 用例编号：TestCase_AcuHMI_001_05_case01_2
# 用例标题：接入设备 Datalog 栏，通过日志产生时间区间和间隔
#          （1day、7days、1month），可下载日志到本地 PC，导出日志格式为 .csv
# 预置条件：
#   1. AcuHMI 上电
#   2. 设备已接入并在线；用例原文要求已采集超过 1 个月
# 测试步骤：
#   2-7. 选中有 datalog 的时间段，依次选择 1day/7days/1month 间隔，
#        点击导出并检查文件内容和格式
# 预期结果：各间隔导出文件数据时间戳及时间间隔正确
# 设计约束：Log Interval 必须小于数据时长；数据跨度不足的档位后端拒绝导出，
#      属正常防护，本脚本仅测数据跨度足够的档位，不足档位 skip 待补
#      （用例原文要求已采集超过 1 个月，数据累计满足后本脚本即完整校验）。

_INTERVALS = ("1 day", "7 days", "1 month")


def test_TestCase_AcuHMI_001_05_case01_2(app_page: Page, tmp_path: Path):
    page = app_page
