# -*- coding: utf-8 -*-
"""
用例 ID：TestCase_AcuHMI_003_08_case09
类别：A — Post Historical Data JSON 推送（FTP，UTC Timestamp 文件名）
功能：配置 Post Channel 1（FTP），JSON 格式，Log File Length=1 min，Log Interval=1 min，
...
【测试步骤】
  ...
  5. Log File Name Format = UTC Timestamp
  6. Log File Name Prefix = meter0_logger1
  7. Log File Length = 1 minute
  8. Log Interval = 1 minute
  9. 点击 Post，等待进度框到 100%
  10. 检查 FTP 目录收到的文件

【预期结果】
  - FTP 目录下出现 .json 文件
  - 文件名格式符合 UTC Timestamp 规则，前缀为 meter0_logger1
  - JSON 中时间戳字段为 UTC Seconds（Unix 时间戳）格式
  - Log Interval 约 60 s，Log File Length 约 1 min
"""
from helpers import run_post_historical_case

CASE_ID = "TestCase_AcuHMI_003_08_case09"
