# -*- coding: utf-8 -*-
"""
用例 ID：TestCase_AcuHMI_003_03_case02
类别：A — Disable 验证
功能：Logger3 处于 Disable 状态时，HTTP / HTTPS 接收目录不应出现任何文件
...
【测试步骤】
  1. 进入 Data Loggers 页面，将 Logger3 设为 Disable 并保存
  2. 等待 90 秒，覆盖至少一个文件推送窗口
  3. 检查 HTTP / HTTPS 协议数据接收目录下的 .csv / .json 文件

【预期结果】
  - HTTP 接收目录下不出现任何文件
  - HTTPS 接收目录下不出现任何文件（如已启用）
"""
import time
from helpers import collect_files
from datalog_page import DataLoggerPage

CASE_ID      = "TestCase_AcuHMI_003_03_case02"
LOGGER_N     = 3
