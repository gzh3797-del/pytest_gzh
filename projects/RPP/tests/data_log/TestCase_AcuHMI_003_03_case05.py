# -*- coding: utf-8 -*-
"""
用例 ID：TestCase_AcuHMI_003_03_case05
类别：C — 正向推送验证
功能：Logger3 通过 SFTP 推送 CSV 文件，验证文件到达及格式正确性
...
【测试步骤】
  1. 进入 Data Loggers 页面，对 Logger3 进行如下配置并保存：
       ...
       Log File Name Prefix = meter1_Logger3
       Log Interval         = 1 minute
  2. 等待最长 450 秒，轮询 SFTP 接收目录直至出现 .csv 文件
  3. 验证文件扩展名为 .csv
  4. 验证文件名格式：meter1_Logger3 + 开始时间戳 + 结束时间戳（Time Interval Format）
  5. 验证文件内容中时间戳列为 UTC 秒数（10~13 位纯数字）
  6. 验证相邻行时间戳间隔约为 1 minute（±10%）
  7. 验证文件数据覆盖时长约为 5 minute（±10%）
  8. 将 Logger3 设为 Disable

【预期结果】
  - SFTP 接收目录内收到至少一个 .csv 文件
  - 文件名符合 Time Interval Format（meter1_Logger3<start><end>.csv）
  - 所有时间戳列为 UTC 秒数格式
...
CASE_ID      = "TestCase_AcuHMI_003_03_case05"
LOGGER_N     = 3
PROTOCOL     = "SFTP"
FILE_FORMAT  = "csv"
