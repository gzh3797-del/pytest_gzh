...
_PROJECT_ROOT = _HERE.parent.parent  # RPP/
sys.path.insert(0, str(_PROJECT_ROOT))
sys.path.insert(0, str(_HERE))  # 优先级最高，确保本地 config.py 覆盖其他同名模块

import config

# 仓库根目录入 path，便于 import projects.RPP.helpers（对齐 BacnetIP conftest）
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    html 路径，使 data_log 与其它模块一样在 reports/RPP/data_log/<时间戳>/report.html
    ...
    base = Path(report_root) if report_root else (
        Path(_REPO_ROOT) / "reports" / "RPP" / "data_log" / ts
    )
