...
def _discover_devices():
    ...
    复用 projects/RPP/helpers/physical_devices_reader（本项目内，非跨项目）。
    session 内只发现一次并缓存；page 未注入或发现失败时返回 None → 调用方回退旧逻辑
    （mib_mapping 选名 + devices.yaml 连接）。

    注意：该 reader 以 projects.RPP.settings.HMI_URL 作网关基址，运行时需保证 HMI_URL
    指向所测网关（与本 session 登录的是同一台，与 parameter_settings 一致）。
    """
    ...
    try:
        ...
        from projects.RPP.helpers.physical_devices_reader import discover_modbus_devices
