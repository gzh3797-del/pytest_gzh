...
运行：
  pytest projects/RPP/tests/BacnetIP/test_bacnet_ui_config.py -v
...
def _navigate_to_bacnet(page: Page) -> None:
    ...
    page.evaluate(
        """() => {
            // RPP：点击「Settings」顶级导航组（协议页归属此组），精确匹配避开 'System Settings'
            for (const el of document.querySelectorAll('.nav-item, .nav-item-menu, header span')) {
