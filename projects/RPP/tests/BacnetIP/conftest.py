...
# ── 设备 Web UI 配置（统一维护于 projects/RPP/settings.py）────────────
from projects.RPP.settings import get_screenshot_dir  # noqa: E402
from projects.RPP.pages.login_page import LoginPage  # noqa: E402
...
def _login_and_navigate_bacnet(page: Page) -> None:
    ...
    # ── 切换到 RPP「Settings」顶级导航组（协议页归属此组；JS click 绕过 overlay）──
    # RPP 把 HMI 单一设备菜单拆成 About/Commissioning/Monitoring/Settings 顶级组，
    # BACnet/IP 在 Settings → Protocols 下。精确匹配 'Settings'，避开 'System Settings' 等子项。
    page.evaluate(
        """() => {
            for (const el of document.querySelectorAll('header span, .nav-item, .nav-item-menu')) {
                if (el.textContent.trim() === 'Settings') { el.click(); return; }
            }
        }"""
    )
