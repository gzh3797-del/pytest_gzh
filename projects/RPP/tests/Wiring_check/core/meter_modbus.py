...
class WiringCheckModbus:
    def __init__(self):
        if cfg.METER_TCP_IP is None or cfg.METER_TCP_PORT is None or cfg.MODBUS_SLAVE is None:
            raise RuntimeError(
                "被测表连接参数未就绪（cfg.METER_TCP_IP/PORT/MODBUS_SLAVE 为 None）。"
                "请确保 meter_connection fixture 已先运行（pytest 路径），"
                "或在独立运行时先调 cfg.ensure_meter_connection()。"
            )
