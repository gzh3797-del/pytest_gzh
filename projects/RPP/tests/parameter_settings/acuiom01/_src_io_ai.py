# -*- coding: utf-8 -*-
"""
_src_io_ai.py — AcuIOM-1 "IO → AI" 页面操作

页面路径：Settings → Devices → <DEVICE_NAME>（span.link-url）→ 详情页顶部
Meter|IO 切换按钮 → 点 IO → 二级标签 AI
URL hash（探查实测样例，随设备 hash 变化）：
  .../#/device/connection/deviceSetting/io/<设备hash>/<idx>/0/0

页面结构（探查实测，https://192.168.3.47，Element Plus v2）：
  单张 <table>，每行：checkbox(0) / ID(1,"AI n") / Signal Type(2,el-select) /
  Input Lower Limit(3,input) / Input Upper Limit(4,input) / Eng. Unit(5,input) /
  Action(6，Edit + Copy 两个按钮)
  行内 Edit 按钮弹出 .el-dialog "AI n Details"：
    Signal Type(el-select，同 4 选项) / Input Lower Limit(input) /
    Input Upper Limit(input) / Eng. Unit(input) /
    Number of Segments(el-select，选项 1/2/3) /
    Breakpoints 表：Point 1~4，列序 "Input (V)" -> "Eng. Value (x)"；
      Segments=2 时 Point4 两个 input 均带 disabled（实测确认，Segments=1/3
      未逐一验证，按线性关系推断 Segments=k 时 Point(k+1)..4 disabled）
    Cancel / Confirm 按钮
  顶部 Apply to Selected / Reset Selected（默认 disabled，需先勾选至少一行）；
  每行 Copy 点击后顶部提示文案变为 "AI n Settings Copied!"
  底部固定 Save 按钮。
"""
...
from helpers_iom01 import step  # noqa: F401  统一 step() 日志/Allure 记录
...
def nav_to_io(page: Page) -> None:
    ...
    from helpers_iom01 import DEVICE_NAME
