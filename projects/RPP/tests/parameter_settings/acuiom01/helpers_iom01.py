# -*- coding: utf-8 -*-
"""
helpers_iom01.py — AcuIOM-1（8 路 AI + 2 路 AO）页面操作 + Modbus 校验公共函数
...
寄存器来源：knowledge/shared/templates/raw/AcuIOM-01_v1.01_20250205.xlsx（blockParams sheet）
...
# ── 设备连接常量：由 conftest.py 的 _bind_acuiom01_device（autouse，会话级）
# 在测试开始前通过网关 API 动态发现当前在线的 AcuIOM-1 设备后写入真实值，
...
# 该编码顺序按页面下拉展示顺序假定为 0/1/2/3。建议真机跑通 _001_001(冒烟)/
# _001_002(四种类型遍历) 用例时用实际回读值反推校正本表。
SIGNAL_TYPE_ENCODE = {
    "Voltage(0-10V)":  0,
    "Voltage(2-10V)":  1,
    "Current(0-20mA)": 2,
...
_log = logging.getLogger("acuiom01_test")
...
def read_unit_ascii(address: int, num_regs: int = 2) -> str:
    ...
    ASCII 打包模式（跨传输回读时滤除 0x00 再解码）**推测**实现——探查阶段只
    看到页面自由文本框（示例值 "x"/"kw"/"Cm"），未做 Modbus 回读比对验证，
    请在真机上用本函数实测后确认/修正打包方式（是否 ASCII、是否本函数假定
    的字节序）。
    """
...
# 让 from helpers_iom01 import * 包含 _log 等下划线名称（须放文件最末）
