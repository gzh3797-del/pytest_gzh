# -*- coding: utf-8 -*-
"""
_src_io_di.py — AcuIOM-3 "IO → DI" 页面操作

页面结构与 acuiom04/_src_io_di.py 完全同构（同一前端组件，仅通道数不同：14 DI）。
详见 acuiom04/_src_io_di.py 顶部说明。**设备当前离线，本模块函数在离线状态下
不会被调用（conftest.py 会 pytest.skip 整组用例）**。
"""
...
def nav_to_io(page: Page) -> None:
    ...
    from helpers_iom03 import DEVICE_NAME
