...
pytest
  └─ Playwright 打开网关 Web UI（https://<GATEWAY_IP>）
       └─ 登录 → 导航至 Gateway Devices → <设备> → Settings → General
            └─ 填写参数值 → 点击 Save（参数下发）
...
#### 3.1.1 User and CT 页面（+15 条，接入设备参数设置）

同目录另含 15 条「User and CT」页面用例（`_src_user_and_ct.py` + `test_Testcase_AcuHMI_001_07_case38~42` / `..._032_002_003~015`）：

| 用例组 | 内容 | 验证方式 |
|--------|------|---------|
| 接线方式（001_07_case38~42，5 条） | 5 种 Wiring 下 Voltage Assignment 约束、VA 只读性、User Channel 区块显隐/Phase 行为 | 切 Wiring（自动保存，等 Running）→ Modbus 回读 4162/VA 寄存器 + 页面断言 |
| User Channel 命名（032_002_003/006/008~015，10 条） | Description 独立命名/空值回退/ASCII 校验/长度边界/重名/立即生效/跨页传播/持久化 | 填 Description→Save→Modbus 回读 4608 系列 + 失败断言 `.el-message--warning` toast |

> 未纳入：032_002_004（首启默认名）/_005（恢复出厂）/_007（旧配置迁移）——需破坏性/重环境操作，暂留手工。
> 选择器与真机行为沉淀见 `knowledge/meters/RPP/requirements/context/UserAndCT_context.md`。
