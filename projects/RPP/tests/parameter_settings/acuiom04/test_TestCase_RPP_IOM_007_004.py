# -*- coding: utf-8 -*-
"""
TestCase_RPP_IOM_007_004
模块：IO接入参数设置 → AcuIOM-04 DO设置
标题：DO Copy 批量复制 + Reset 恢复出厂

步骤：
  1. 进入 IO-DO 标签
  2. 前置：目标 DO2 Control Mode 设为 Pulse、源 DO1 设为 Manual（制造差异），分别保存
  3. 勾选源 DO1 → Copy → 勾选目标 DO2 → Apply to Selected → Save
  4. Modbus 回读 DO2 Control Mode，确认被套用为 DO1 的 Manual
  5. 还原：勾选 DO1、DO2 → Reset Selected 恢复出厂
...
备注：Control Mode 完整选项集未全探（已知 Manual/Pulse）；Pulse Width 一并被复制，
...
def test_TestCase_RPP_IOM_007_004(app_page):
