# -*- coding: utf-8 -*-
"""
TestCase_RPP_IOM_006_006
模块：IO接入参数设置 → AcuIOM-04 DI设置
标题：DI Copy 批量复制 + Reset 恢复出厂（覆盖至 DI28 验 28 通道）

步骤：
  1. 进入 IO-DI 标签
  2. 前置：源 DI1 Function 设为 Pulse Counter（区别于默认 Status Monitor），保存
  3. 勾选源 DI1 → Copy → 勾选目标 DI28 → Apply to Selected → Save
  4. Modbus 回读 DI28 Function，确认被套用为 DI1 的 Pulse Counter
  5. 还原：DI28 Reset 出厂；DI1 设回 Status Monitor
...
def test_TestCase_RPP_IOM_006_006(app_page):
    ...
    select_row_checkbox(app_page, DI_COUNT)
    click_apply_to_selected(app_page)
    save_and_check(app_page)  # 相同配置不提示成功，以 Modbus 回读为准

    verify_modbus(di_type_reg(DI_COUNT), FUNCTION_ENCODE["Pulse Counter"],
                  label=f"DI{DI_COUNT} Function(applied from DI1)")
