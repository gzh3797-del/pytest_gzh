# -*- coding: utf-8 -*-
"""
helpers_iom02.py — AcuIOM-2（16 路 AI + 4 路 AO）页面操作 + Modbus 校验公共函数

参照 acurev4100/helpers_4100.py 的结构与约定（team conventions #9/#12）；
寄存器基址/跨度与 acuiom01/helpers_iom01.py 完全一致（已用
knowledge/shared/templates/raw/AcuIOM-02_v1.01_20250205.xlsx 核实两台模板的
blockParams 基址相同，仅通道数量不同）。

页面路径：Settings → Devices → <设备>（span.link-url） → 详情页顶部 Meter|IO 切换
→ IO → AI / AO 二级标签
探查环境：https://192.168.3.47（ACM-41-WEB2 Gateway Mode，账号 l/1）

寄存器来源：knowledge/shared/templates/raw/AcuIOM-02_v1.01_20250205.xlsx（blockParams sheet）
  AI 配置块基址 12288，每路跨度 22 regs：
    +0  AIn Type            uint16   (Signal Type 编码)
    +1  AIn Top Limit       uint32×0.001  (Input Upper Limit)
    +3  AIn Bot Limit       uint32×0.001  (Input Lower Limit)
    +5  AIn Line Num        uint16   (Number of Segments)
    +6/8/10/12   AIn Line PointX1-4  uint32×0.001  (Breakpoints "Input (V)" 列)
    +14/16/18/20 AIn Line PointY1-4  int32×0.001   (Breakpoints "Eng. Value (x)" 列)
  AI Eng. Unit 块基址 12800，每路跨度 2 regs（uint32）：AIn Unit
  AO 配置块基址 13312，每路跨度 22 regs（结构同 AI，但断点列序相反）：
    +0 AOn Type / +1 Top Limit(Output Upper) / +3 Bot Limit(Output Lower) / +5 Line Num
    +6/8/10/12   AOn Line PointX1-4  int32×0.001   (Breakpoints "Eng. Value (x)" 列，AO 与 AI 相反)
    +14/16/18/20 AOn Line PointY1-4  uint32×0.001  (Breakpoints "Output (V)" 列)
  AO Eng. Unit 块基址 13472，每路跨度 2 regs（uint32）：AOn Unit
"""
...
# ── 设备连接常量：由 conftest.py 的 _bind_acuiom02_device（autouse，会话级）
# 在测试开始前通过网关 API 动态发现当前在线的 AcuIOM-2 设备后写入真实值，
...
REG_AI_BASE = 12288
AI_STRIDE = 22
REG_AI_UNIT_BASE = 12800
AI_UNIT_STRIDE = 2


def ai_type_reg(n: int) -> int:
    """AI n(1-based) Type 寄存器（Signal Type 编码，uint16）。"""
    return REG_AI_BASE + (n - 1) * AI_STRIDE


def ai_top_limit_reg(n: int) -> int:
    """AI n Top Limit（Input Upper Limit，uint32×0.001）。"""
    return ai_type_reg(n) + 1


def ai_bot_limit_reg(n: int) -> int:
    """AI n Bot Limit（Input Lower Limit，uint32×0.001）。"""
    return ai_type_reg(n) + 3


def ai_line_num_reg(n: int) -> int:
    """AI n Line Num（Number of Segments，uint16）。"""
    return ai_type_reg(n) + 5


def ai_point_x_reg(n: int, point: int) -> int:
    """AI n 断点 point(1~4) 的 X 列（页面 "Input (V)"，uint32×0.001）。"""
    return ai_type_reg(n) + 6 + (point - 1) * 2


def ai_point_y_reg(n: int, point: int) -> int:
    """AI n 断点 point(1~4) 的 Y 列（页面 "Eng. Value (x)"，int32×0.001）。"""
    return ai_type_reg(n) + 14 + (point - 1) * 2


def ai_unit_reg(n: int) -> int:
    """AI n Unit（Eng. Unit，uint32，2 regs）。"""
    return REG_AI_UNIT_BASE + (n - 1) * AI_UNIT_STRIDE


# ── AO 配置寄存器（结构同 AI，断点列序相反） ─────────────────────────
REG_AO_BASE = 13312
AO_STRIDE = 22
REG_AO_UNIT_BASE = 13472
AO_UNIT_STRIDE = 2


def ao_type_reg(n: int) -> int:
    """AO n(1-based) Type 寄存器（Signal Type 编码，uint16）。"""
    return REG_AO_BASE + (n - 1) * AO_STRIDE


def ao_top_limit_reg(n: int) -> int:
    """AO n Top Limit（Output Upper Limit，uint32×0.001）。"""
    return ao_type_reg(n) + 1


def ao_bot_limit_reg(n: int) -> int:
    """AO n Bot Limit（Output Lower Limit，uint32×0.001）。"""
    return ao_type_reg(n) + 3


def ao_line_num_reg(n: int) -> int:
    """AO n Line Num（Number of Segments，uint16）。"""
    return ao_type_reg(n) + 5


def ao_point_x_reg(n: int, point: int) -> int:
    """AO n 断点 point(1~4) 的 X 列（页面 "Eng. Value (x)"，int32×0.001，与 AI 相反）。"""
    return ao_type_reg(n) + 6 + (point - 1) * 2


def ao_point_y_reg(n: int, point: int) -> int:
    """AO n 断点 point(1~4) 的 Y 列（页面 "Output (V)"，uint32×0.001，与 AI 相反）。"""
    return ao_type_reg(n) + 14 + (point - 1) * 2


def ao_unit_reg(n: int) -> int:
    """AO n Unit（Eng. Unit，uint32，2 regs）。"""
    return REG_AO_UNIT_BASE + (n - 1) * AO_UNIT_STRIDE


# ── 下拉编码映射 ─────────────────────────────────────────────────────
# TODO(需真机确认)：探查阶段只做了只读页面探查（未做 Modbus 回读联动验证），
# 该编码顺序按页面下拉展示顺序假定为 0/1/2/3。建议真机跑通 _003_001(冒烟)/
# _003_002(四种类型遍历) 用例时用实际回读值反推校正本表。
SIGNAL_TYPE_ENCODE = {
    "Voltage(0-10V)":  0,
    "Voltage(2-10V)":  1,
    "Current(0-20mA)": 2,
...
_log = logging.getLogger("acuiom02_test")
...
def modbus_read_i32(address: int) -> int:
    """读取 32-bit 有符号整数（两补码）。"""
    val = modbus_read_u32(address)
    if val >= 0x8000_0000:
        val -= 0x1_0000_0000
    return val
...
def verify_modbus_u32(address: int, expected: int, label: str = "") -> None:
    actual = modbus_read_u32(address)
    ok = actual == expected
...
def verify_modbus_scaled(
    ...
    """验证 ×scale 缩放的 32-bit 寄存器（uint32 或 int32）。"""
    raw = modbus_read_i32(address) if signed else modbus_read_u32(address)
    actual = raw * scale
...
# ── 页面操作：通用（label-based，供 Edit 弹窗内字段使用） ────────────
...
# 让 from helpers_iom02 import * 包含 _log 等下划线名称（须放文件最末）
