...
class MQTTPage(BasePage):
    ...
    # 补充 "MQTT Enable"：RPP 的 General tab 在 MQTT Disable（默认状态）时 Broker 等字段
    # 全部隐藏，只有 "MQTT Enable" 一直可见，仅靠 Broker label 会把"已到页但未启用"误判为
    # "未到页"。
    MQTT_PAGE_MARKER = (By.XPATH,
        ...
        " | //label[contains(normalize-space(.),'MQTT Enable')]")
                ...
                if mqtt_el is not None:
                    ...
                    if not self._page_loaded(timeout=2):
                        ...
                        if sidebar_gen:
                            ...
                            time.sleep(1.5)
                    # RPP：MQTT 是水平菜单里的 el-sub-menu__title，子项（General/User
                    # Credential/…）靠真实鼠标悬停（hover）才会以 popper 下拉形式渲染出来，
                    # click（含 JS click）不会触发；上面两步都没进入 MQTT 页时，用
                    # ActionChains 做一次真实 hover 再找 "General"。
                    if not self._page_loaded(timeout=2):
                        try:
                            ActionChains(self.driver).move_to_element(mqtt_el).perform()
                            time.sleep(0.8)
                        except Exception:
                            pass
                        hover_gen = self.driver.find_elements(By.XPATH,
                            "//li[contains(@class,'el-menu-item')"
                            " and normalize-space(.)='General']")
                        if hover_gen:
                            log.info("  → hover 展开 MQTT 下拉，点击 General 进入")
                            self._js_click(hover_gen[0])
                            time.sleep(1.5)
                    return True
