...
> ⚠️ 本目录从 `projects/AcuHMI_1_7/tests/mqtt/` 迁移而来。外层配置
> （`projects/RPP/settings.py`/`config.yaml`/`conftest.py`）已指向 RPP 真机
> （`http://192.168.2.94:3030`），但本文档以及 `test_mqtt.py`/`mqtt_page.py`
> 的用例主体仍按 AcuHMI-1-7/WEB2 页面结构描述，尚未对照 RPP 真机现场验证，
> 下文"项目切换（WEB2 ↔ HMI1-7）"一节在 RPP 里已不适用（RPP 没有该切换机制）。
