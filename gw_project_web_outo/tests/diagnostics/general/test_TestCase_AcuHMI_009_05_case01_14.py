...
# 用例编号：TestCase_AcuHMI_009_05_case01_14
# 用例标题：使能Modbus调试跟踪，间隔时间1周，类型RTU_REQ，slaveid为-1（无效值），功能码22，点击搜索日志为空，点击重置筛选条件被清除
# 预置条件：1、管理权限登录AcuHMI网页
# 测试步骤：
#   1. Diagnostics->Modbus Debug Log设置Modbus Debug Trace为Enable
#   2. 设置间隔时间为1周，类型选择RTU_REQ，slaveid为-1，功能码为22
#   3. 点击Search
#   4. 点击Reset
# 预期结果：
#   3. 日志信息为空
#   4. 调试日志筛选条件被清除
@pytest.mark.xfail(strict=False, reason="Product does not reject invalid slaveid=-1; search returns all logs instead of empty")
def test_TestCase_AcuHMI_009_05_case01_14(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    # Fill slave ID with invalid value -1
    page.get_by_placeholder("Slave ID").fill("-1")
    page.wait_for_timeout(200)

    page.get_by_role("button", name="Search").click()
    page.wait_for_timeout(1000)

    # Results should be empty for invalid slaveid=-1
    ...
    assert row_count == 0 or no_data_visible, \
        f"slaveid=-1无效值时，搜索结果应为空，当前行数：{row_count}"
