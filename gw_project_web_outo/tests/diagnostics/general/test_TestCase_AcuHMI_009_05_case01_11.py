...
# 用例编号：TestCase_AcuHMI_009_05_case01_11
# 用例标题：Modbus Debug Log启用，1周/TCP_RSP/slaveid:246/fc16，验证日志字段，Reset筛选
@pytest.mark.xfail(strict=False, reason="Modbus Debug Log筛选结果依赖实际Modbus通信数据")
def test_TestCase_AcuHMI_009_05_case01_11(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            page.locator("input[placeholder*='slave'], input[placeholder*='Slave']").first.fill("246")
        except Exception:
            pass

    # Enter function code
    try:
        page.locator(".el-form-item").filter(has_text="Function Code").locator("input").fill("16")
    except Exception:
        try:
            page.locator("input[placeholder*='function'], input[placeholder*='Function']").first.fill("16")
        except Exception:
            pass
