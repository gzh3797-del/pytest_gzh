...
# 用例编号：TestCase_AcuHMI_009_05_case01_5
# 用例标题：Modbus Debug Log启用，1年/RTU_RSP/slaveid:99/fc3，验证日志字段，Reset筛选
@pytest.mark.xfail(strict=False, reason="Modbus Debug Log筛选结果依赖实际Modbus通信数据")
def test_TestCase_AcuHMI_009_05_case01_5(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        page.get_by_role("option", name="RTU_RSP", exact=True).click()
        page.wait_for_timeout(200)
    except Exception:
        try:
            page.locator("select").filter(has_text="Type").select_option("RTU_RSP")
            page.wait_for_timeout(200)
        except Exception:
            pass
    ...
    # Enter slave ID
    try:
        page.locator(".el-form-item").filter(has_text="Slave ID").locator("input").fill("99")
    except Exception:
        try:
            page.locator("input[placeholder*='slave'], input[placeholder*='Slave']").first.fill("99")
        except Exception:
            pass
