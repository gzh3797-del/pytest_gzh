...
# 用例编号：TestCase_AcuHMI_009_05_case01_6
# 用例标题：Modbus Debug Log启用，1天/TCP_REQ/slaveid:100/fc5，验证日志字段，Reset筛选
@pytest.mark.xfail(strict=False, reason="Modbus Debug Log筛选结果依赖实际Modbus通信数据")
def test_TestCase_AcuHMI_009_05_case01_6(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        page.get_by_role("option", name="TCP_REQ", exact=True).click()
        page.wait_for_timeout(200)
    except Exception:
        try:
            page.locator("select").filter(has_text="Type").select_option("TCP_REQ")
            page.wait_for_timeout(200)
        except Exception:
            pass
    ...
    # Enter slave ID
    try:
        page.locator(".el-form-item").filter(has_text="Slave ID").locator("input").fill("100")
    except Exception:
        try:
            page.locator("input[placeholder*='slave'], input[placeholder*='Slave']").first.fill("100")
        except Exception:
            pass

    # Enter function code
    try:
        page.locator(".el-form-item").filter(has_text="Function Code").locator("input").fill("5")
    except Exception:
        try:
            page.locator("input[placeholder*='function'], input[placeholder*='Function']").first.fill("5")
        except Exception:
            pass
