...
# 用例编号：TestCase_AcuHMI_009_05_case01_3
# 用例标题：Modbus Debug Log启用，1周/TCP_RSP/slaveid:2/fc2，验证日志字段，Reset筛选
@pytest.mark.xfail(strict=False, reason="Modbus Debug Log筛选结果依赖实际Modbus通信数据")
def test_TestCase_AcuHMI_009_05_case01_3(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            page.locator("select").filter(has_text="Type").select_option("TCP_RSP")
            page.wait_for_timeout(200)
        except Exception:
            pass
    ...
    # Enter slave ID
    try:
        page.locator(".el-form-item").filter(has_text="Slave ID").locator("input").fill("2")
    except Exception:
        try:
            page.locator("input[placeholder*='slave'], input[placeholder*='Slave']").first.fill("2")
        except Exception:
            pass

    # Enter function code
    try:
        page.locator(".el-form-item").filter(has_text="Function Code").locator("input").fill("2")
    except Exception:
        try:
            page.locator("input[placeholder*='function'], input[placeholder*='Function']").first.fill("2")
        except Exception:
            pass
