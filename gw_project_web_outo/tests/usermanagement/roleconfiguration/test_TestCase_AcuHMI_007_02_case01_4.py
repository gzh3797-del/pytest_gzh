...
_ROLE_NAME = "rc01_4uedit"
_USER_NAME = "rcuser01_4"
...
# 用例编号：TestCase_AcuHMI_007_02_case01_4
# 用例标题：添加角色，角色权限为用户-编辑，其余均为无，创建该用户，登录后查看该用户权限
...
#   1. 保存成功
#   2. 创建成功
#   3. 该用户权限为用户-编辑（可进入 User Configuration，有 Add User 按钮）
def test_TestCase_AcuHMI_007_02_case01_4(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            # 登录成功（User=edit 有权限访问系统）
            assert "/#/login" not in p.url, \
                f"User=edit 角色用户登录应成功，当前 URL: {p.url}"

            # User Configuration 可访问，且为编辑权限（有 Add User 按钮）
