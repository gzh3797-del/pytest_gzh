...
_ROLE_NAME = "rc01_9ssv"
_USER_NAME = "rcuser01_9"
_INIT_PWD  = "Admin@110001"

# System Settings=view, all others=none
_PERM_MAP = {
    "User":             "none",
    "Device":           "none",
    "Data Log":         "none",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_9
# 用例标题：添加角色，角色权限为系统设置-视图，其余均为无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration -> Add Role，System Settings=view，其余均为 none
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   3. 该用户权限为系统设置-视图（落地页为 System Settings/Date & Time 页面，无 Save 按钮）
def test_TestCase_AcuHMI_007_02_case01_9(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            # 登录成功，落地页为 System Settings 页面
            assert "/#/login" not in p.url, \
                f"System Settings=view 角色用户登录应成功，当前 URL: {p.url}"
            assert "systemSettings" in p.url, \
                f"System Settings=view 角色用户应落地于 System Settings 页面，当前 URL: {p.url}"

            # view 权限：无 Save 按钮（也无 Sync 按钮）
            save_btn = p.get_by_role("button", name="Save")
            assert not save_btn.is_visible(), \
                "System Settings=view 角色用户不应有 Save 按钮"
        finally:
            ctx.close()
    finally:
