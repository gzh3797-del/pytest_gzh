...
_ROLE_NAME = "rc01_11pv"
_USER_NAME = "rcuser01_11"
_INIT_PWD  = "Admin@110001"

# Protocol=view, all others=none
_PERM_MAP = {
    "User":             "none",
    "Device":           "none",
    "Data Log":         "none",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_11
# 用例标题：添加角色，角色权限为协议-视图，其余均为无，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration -> Add Role，Protocol=view，其余均为 none
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统
# 预期结果：
#   3. 该用户权限为协议-视图（落地页为 Protocol 页，无 Save 按钮）
def test_TestCase_AcuHMI_007_02_case01_11(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            # 登录成功，落地页为 Protocol 页
            assert "/#/login" not in p.url, \
                f"Protocol=view 角色用户登录应成功，当前 URL: {p.url}"
            assert "protocols" in p.url, \
                f"Protocol=view 角色用户应落地于 Protocol 页，当前 URL: {p.url}"

            # view 权限：无 Save 按钮
            save_btn = p.get_by_role("button", name="Save")
            assert not save_btn.is_visible(), \
                "Protocol=view 角色用户在 Protocol 页不应有 Save 按钮"
        finally:
            ctx.close()
    finally:
