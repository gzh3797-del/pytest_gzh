...
_ROLE_NAME = "rc01_3uview"
_USER_NAME = "rcuser01_3"
...
# 用例编号：TestCase_AcuHMI_007_02_case01_3
# 用例标题：添加角色，角色权限为用户-视图，其余均为无，创建该用户，登录后查看该用户权限
...
#   1. 保存成功
#   2. 创建成功
#   3. 该用户权限为用户-视图（可进入 User Configuration，无 Add User 按钮）
def test_TestCase_AcuHMI_007_02_case01_3(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            # 登录成功（User=view 有权限访问系统）
            assert "/#/login" not in p.url, \
                f"User=view 角色用户登录应成功，当前 URL: {p.url}"

            # User Configuration 可访问，但为视图权限（无 Add User 按钮）
