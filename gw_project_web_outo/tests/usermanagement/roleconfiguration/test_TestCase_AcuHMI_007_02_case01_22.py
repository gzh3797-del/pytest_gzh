...
_ROLE_NAME = "rc01_22fwe"
_USER_NAME = "rcuser01_22"
...
# 用例编号：TestCase_AcuHMI_007_02_case01_22
# 用例标题：添加角色，角色权限为固件更新-编辑，其余均为无，创建该用户，登录后查看该用户权限
...
#   3. 新用户登录系统
# 预期结果：
#   3. 该用户权限为固件更新-编辑（落地页为 Firmware Update 页，有 Browse/Upload 按钮）
def test_TestCase_AcuHMI_007_02_case01_22(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            assert "/#/login" not in p.url, \
                f"Firmware Update=edit 角色用户登录应成功，当前 URL: {p.url}"
            assert "firmwareUpdate" in p.url, \
                f"Firmware Update=edit 角色用户应落地于 Firmware Update 页，当前 URL: {p.url}"

            expect(p.get_by_role("button", name="Browse")).to_be_visible(timeout=5000)
        finally:
            ctx.close()
    finally:
