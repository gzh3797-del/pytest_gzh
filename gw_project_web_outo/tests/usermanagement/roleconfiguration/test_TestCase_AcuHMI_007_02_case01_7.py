...
_ROLE_NAME = "rc01_7dlv"
_USER_NAME = "rcuser01_7"
_INIT_PWD  = "Admin@110001"

# Data Log=view, all others=none
_PERM_MAP = {
    "User":             "none",
    "Device":           "none",
    "Data Log":         "view",
...
# 用例编号：TestCase_AcuHMI_007_02_case01_7
# 用例标题：添加角色，角色权限为数据记录-视图，其余均为无，创建该用户，登录后验证权限限制
# 测试步骤：
#   1. Role Configuration -> Add Role，Data Log=view，其余均为 none
#   2. User Configuration -> Add User，角色设置为该自定义角色
#   3. 新用户登录系统，落地到 Data Log 页面
#   4. 点击顶部导航 "About"
# 预期结果：
#   3. 登录成功，落地于 /#/dataLog/... 页面
#   4. 弹出 "No Any Permissions" 提示（用户无权限访问 About 区域）
def test_TestCase_AcuHMI_007_02_case01_7(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        try:
            ...
            # Step 3: 登录应成功，落地于 Data Log 页面
            assert "/#/login" not in p.url, \
                f"Data Log=view 用户登录应成功，当前 URL: {p.url}"
            assert "dataLog" in p.url, \
                f"Data Log=view 用户应落地于 Data Log 页面，当前 URL: {p.url}"

            # Step 4: 点击 "About" → 应弹出 "No Any Permissions" toast
            p.locator("header span").filter(has_text="About").first.click()
            toast = p.locator(".el-message__content").filter(has_text="No Any Permissions")
            toast.wait_for(state="visible", timeout=5000)
            assert toast.is_visible(), \
                "点击 About 后应弹出 'No Any Permissions' 提示（用户无权限访问该区域）"
        finally:
            ctx.close()
    finally:
