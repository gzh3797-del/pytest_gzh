...
# 用例编号：TestCase_AcuHMI_007_02_case02
# 用例标题：编辑角色，由角色权限全无改为全视图，创建该用户，登录后查看该用户权限
# 预置条件：服务启动正常，账号登录成功，进入用户设置界面
# 测试步骤：
#   1. Role Configuration 创建全无权限角色并绑定用户
#   2. 编辑该角色，将所有权限改为 view
#   3. test 用户登录系统
# 预期结果：
#   3. 登录成功，系统权限为视图（User Configuration 无 Add User 按钮）
def test_TestCase_AcuHMI_007_02_case02(login_page: LoginPage):
    login_page.open()
    login_page.login()
