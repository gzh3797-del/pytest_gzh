...
_ROLE_NAME = "pp01_6role"
_USER_NAME = "pp01_6user"
_INIT_PWD  = "Admin@110001"
# 密码含大写+小写+数字（满足 Upper+Lower 和 Numbers+Letters 双重策略）
_GOOD_PWD  = "Abc12345"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_6
# 用例标题：配置密码策略，勾选大写和小写、数字和字母，创建用户密码包含大小写和数字，可登录成功
# 测试步骤：
#   1. Password Policy 勾选 Upper+Lower 和 Numbers+Letters（不勾选 Special Characters），保存
#   2. 创建用户，密码为 "Abc12345"（含大写+小写+数字）
#   3. 新用户登录系统
# 预期结果：
#   1. 提示 "Password policy configuration saved"
#   2. 添加成功
#   3. 登录成功
def test_TestCase_AcuHMI_007_03_case01_6(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    _create_role(page)
    try:
        # Step 1: 启用 Upper+Lower 和 Numbers+Letters（不勾 Special Characters）
        _set_policy(page, upper_lower=True, numbers_letters=True, special_chars=False)

        # Step 2: 创建用户，密码含大写+小写+数字 → 应成功
        _nav_to_submenu(page, "User Configuration")
        ...
        assert not page.get_by_label("Password", exact=True).is_visible(), \
            "Upper+Lower+Numbers+Letters 策略下，含大小写+数字的密码创建用户应成功（对话框应关闭）"
