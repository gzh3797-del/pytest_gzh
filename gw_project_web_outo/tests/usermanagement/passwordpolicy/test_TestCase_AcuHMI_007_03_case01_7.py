...
_ROLE_NAME = "pp01_7role"
_USER_NAME = "pp01_7user"
# 密码含大写+小写+特殊字符（满足 Upper+Lower 和 Special Characters 双重策略）
_GOOD_PWD  = "Abc@Test"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_7
# 用例标题：配置密码策略，勾选大写和小写、特殊字符，创建用户密码包大写和小写字母、特殊字符，可登录成功
# 测试步骤：
#   1. Password Policy 勾选 Upper+Lower 和 Special Characters（不勾选 Numbers+Letters），保存
#   2. 创建用户，密码为 "Abc@Test"（含大写+小写+特殊字符）
#   3. 新用户登录系统
# 预期结果：
#   1. 提示 "Password policy configuration saved"
#   2. 添加成功
#   3. 登录成功
def test_TestCase_AcuHMI_007_03_case01_7(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    _create_role(page)
    try:
        # Step 1: 启用 Upper+Lower 和 Special Characters（不勾 Numbers+Letters）
        _set_policy(page, upper_lower=True, numbers_letters=False, special_chars=True)

        # Step 2: 创建用户，密码含大写+小写+特殊字符 → 应成功
        _nav_to_submenu(page, "User Configuration")
        ...
        assert not page.get_by_label("Password", exact=True).is_visible(), \
            "Upper+Lower+Special 策略下，含大小写+特殊字符的密码创建用户应成功（对话框应关闭）"
