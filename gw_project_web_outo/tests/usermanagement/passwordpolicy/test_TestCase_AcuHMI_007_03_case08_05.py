...
def _try_set_window_and_assert_fail(page, window_val: str):
    ...
    page.get_by_placeholder("Enter Failed Login Attempt Window").fill(window_val)
    page.get_by_placeholder("Enter Failed Login Wait").fill("300")
    page.get_by_role("button", name="Save").click()
    page.wait_for_timeout(1000)
    ...
    assert has_field_error or has_error_toast or not success_visible, \
        f"Failed Login Attempt Window={window_val} 超出范围，保存应失败"


# 用例编号：TestCase_AcuHMI_007_03_case08_05
# 用例标题：设置失败登录尝试计数为 -1、86401，保存配置失败，提示错误信息正确
# 测试步骤：
#   1. Failed Login Attempt Window=-1，保存 → 应失败
#   2. Failed Login Attempt Window=86401，保存 → 应失败
# 预期结果：
#   保存配置失败，提示错误信息正确
def test_TestCase_AcuHMI_007_03_case08_05(login_page: LoginPage):
    login_page.open()
    login_page.login()
    page = login_page.page

    try:
        _try_set_window_and_assert_fail(page, "-1")
        _try_set_window_and_assert_fail(page, "86401")
    finally:
