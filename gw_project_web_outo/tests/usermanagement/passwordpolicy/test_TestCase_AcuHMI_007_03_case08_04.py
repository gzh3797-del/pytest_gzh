...
# 用例编号：TestCase_AcuHMI_007_03_case08_04
# 用例标题：设置失败登录尝试计数为86400，保存配置成功
# 测试步骤：
#   1. Failed Login Attempt Window=86400（最大有效值），保存
# 预期结果：
#   1. 保存成功
def test_TestCase_AcuHMI_007_03_case08_04(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        page.get_by_placeholder("Enter Failed Login Attempt Window").fill("86400")
        ...
        assert not has_error, \
            "Failed Login Attempt Window=86400（最大有效值）保存应成功，不应出现错误"
    finally:
