...
_ROLE_NAME = "pp01_10role"
_USER_NAME = "pp01_10user"
# 密码仅含特殊字符（无数字和字母），在 Numbers+Letters+Special 策略下应拒绝
_BAD_PWD   = "@@@@@###"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_10
# 用例标题：配置密码策略，勾选数字和字母、特殊字符，创建用户密码包特殊字符，无法创建用户
# 测试步骤：
#   1. Password Policy 勾选 Numbers+Letters 和 Special Characters，保存
#   2. 创建用户，密码为 "@@@@@###"（仅含特殊字符，无数字无字母）
# 预期结果：
#   2. 无法创建用户
def test_TestCase_AcuHMI_007_03_case01_10(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        # Step 2: 创建用户，密码仅含特殊字符（无数字无字母）→ 应失败
        _nav_to_submenu(page, "User Configuration")
        ...
        assert page.get_by_label("Password", exact=True).is_visible(), \
            "Numbers+Letters+Special 策略下，仅含特殊字符的密码创建用户应失败（对话框应保持打开）"
