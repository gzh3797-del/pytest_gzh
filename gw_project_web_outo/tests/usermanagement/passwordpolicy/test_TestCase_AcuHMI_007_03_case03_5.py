...
# 用例编号：TestCase_AcuHMI_007_03_case03_5
# 用例标题：设置最短密码期限为 91，保存配置失败，提示错误信息正确
# 测试步骤：
#   1. Minimum Password Age = 91（超出范围 0-90）
#   2. 点击 Save
# 预期结果：
#   3. 保存配置失败，提示错误信息正确
def test_TestCase_AcuHMI_007_03_case03_5(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        assert (has_field_error or has_error_toast or not success_visible), \
            "Minimum Password Age=91 超出范围（0-90），应保存失败"
    finally:
