...
_ROLE_NAME = "pp05_3role"
_USER_NAME = "pp05_3user"
_SHORT_PWD = "Ac@123"        # 6 chars — too short for min=64
# 64 chars: "Admin@110001" × 5 + "1234" = 60 + 4 = 64
_GOOD_PWD  = "Admin@110001Admin@110001Admin@110001Admin@110001Admin@1100011234"
...
# 用例编号：TestCase_AcuHMI_007_03_case05_3
# 用例标题：设置最小密码长度为 64（最大值），密码长度不足 64 时新增用户保存失败，满足长度时保存成功，且该用户可登录
# 测试步骤：
#   1. Minimum Password Length = 64，保存
#   2. 新增用户，Password 长度 < 64 → 保存失败
#   3. 新增用户，Password 长度 = 64 → 保存成功
#   4. 使用新用户登录
# 预期结果：
#   2. 保存失败
#   3. 保存成功
#   4. 登录成功
def test_TestCase_AcuHMI_007_03_case05_3(login_page: LoginPage):
    login_page.open()
    login_page.login()
