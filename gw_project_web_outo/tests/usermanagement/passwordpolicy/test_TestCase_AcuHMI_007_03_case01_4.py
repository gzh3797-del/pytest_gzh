...
_ROLE_NAME = "pp01_4role"
_USER_NAME = "pp01_4user"
_INIT_PWD  = "Admin@110001"
# 密码含特殊字符（满足 Special Characters 策略，且长度 ≥ 8）
_GOOD_PWD  = "Admin@11"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_4
# 用例标题：配置密码策略，仅勾选特殊字符，创建用户密码包含特殊字符，可登录成功
# 测试步骤：
#   1. Password Policy 仅勾选 Special Characters，保存
#   2. 创建用户，密码为 "Admin@11"（含特殊字符 @）
#   3. 新用户登录系统
# 预期结果：
#   1. 提示 "Password policy configuration saved"
#   2. 添加成功
#   3. 登录成功
def test_TestCase_AcuHMI_007_03_case01_4(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        assert not page.get_by_label("Password", exact=True).is_visible(), \
            "Special Characters 策略下，含特殊字符的密码创建用户应成功（对话框应关闭）"
