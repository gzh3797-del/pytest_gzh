...
_ROLE_NAME = "pp01_8role"
_USER_NAME = "pp01_8user"
# 密码含数字+字母+特殊字符（满足 Numbers+Letters 和 Special Characters 双重策略）
_GOOD_PWD  = "12abc@#!"
...
# 用例编号：TestCase_AcuHMI_007_03_case01_8
# 用例标题：配置密码策略，勾选数字和字母、特殊字符，创建用户密码包数字和字母、特殊字符，可登录成功
# 测试步骤：
#   1. Password Policy 勾选 Numbers+Letters 和 Special Characters（不勾选 Upper+Lower），保存
#   2. 创建用户，密码为 "12abc@#!"（含数字+字母+特殊字符）
#   3. 新用户登录系统
# 预期结果：
#   1. 提示 "Password policy configuration saved"
#   2. 添加成功
#   3. 登录成功
def test_TestCase_AcuHMI_007_03_case01_8(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    _create_role(page)
    try:
        # Step 1: 启用 Numbers+Letters 和 Special Characters（不勾 Upper+Lower）
        _set_policy(page, upper_lower=False, numbers_letters=True, special_chars=True)

        # Step 2: 创建用户，密码含数字+字母+特殊字符 → 应成功
        _nav_to_submenu(page, "User Configuration")
        ...
        assert not page.get_by_label("Password", exact=True).is_visible(), \
            "Numbers+Letters+Special 策略下，含数字+字母+特殊字符的密码创建用户应成功（对话框应关闭）"
