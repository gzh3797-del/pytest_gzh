...
def _user_self_change_pwd(browser, username, current_pwd, new_pwd) -> bool:
    """Non-admin user self-changes password. Returns True on success."""
    ...
    try:
        ...
        return p.get_by_text("password changed", exact=False).is_visible()
    finally:
        ctx.close()
...
def test_TestCase_AcuHMI_007_03_case03_2(login_page: LoginPage):
    ...
    try:
        ...
        changed = _user_self_change_pwd(browser, _USER_NAME, _PWD_0, _PWD_1)
        assert not changed, \
