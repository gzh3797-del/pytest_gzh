...
# 用例编号: TestCase_WEB2_AZU_004_003
# 用例标题: 选择Virtual设备发布数据按间隔上报
...
# 预期结果: 3. 配置成功 | 4. 连接成功，Azure IoT Hub 按配置的 Interval 间隔正常收到Virtual设备参数数据，上报的参数个数和配置的一致，且数值正确
...
def test_TestCase_WEB2_AZU_004_003(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    if device_row.count() > 0:
        ...
        assert False, "测试环境未找到 Virtual 设备"
