...
# 用例编号: TestCase_WEB2_AZU_003_002
# 用例标题: 合法 X509 证书启用 SSL 连接 Azure IoT Hub 成功
# 预置条件: 1.设备已正常上电，相关服务正常启动 | 2.进入 Azure IoT 配置页面，启用 Enable，开启 Enable SSL
# 测试步骤:
#   1. 上传合法 X509 Certificate 文件及匹配的 Key 文件
#   2. 保存配置，查看连接状态及 Azure IoT Hub 接收情况
# 预期结果: 2. Certificate 和 Key 文件上传成功 | 3. SSL 连接建立成功；Azure IoT Hub 正常接收加密上报数据
...
def test_TestCase_WEB2_AZU_003_002(login_page: LoginPage):
    login_page.open()
    login_page.login()
