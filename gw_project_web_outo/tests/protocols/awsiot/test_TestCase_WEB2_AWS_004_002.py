...
# 用例编号: TestCase_WEB2_AWS_004_002
# 用例标题: 4100WEB2选择IOM 设备连接 AWS IoT Core 数据上报
# 预置条件: 1. 设备已正常上电，相关服务正常启动 | 2. 进入 AWS IoT 连接配置页面，启用 Enable | 3. 已配置正确的URL/Topic/Interval/证书文件
# 测试步骤:
#   1. Devices Selection To Mapping 勾选下挂的IOM设备
...
def test_TestCase_WEB2_AWS_004_002(login_page: LoginPage):
    login_page.open()
    login_page.login()
