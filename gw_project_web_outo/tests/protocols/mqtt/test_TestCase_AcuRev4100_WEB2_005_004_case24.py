...
# 用例编号: TestCase_AcuRev4100_WEB2_005_004_case24
# 用例标题: 下拉设备选择AcuAio，选择AI，选择参数状态status，查看客户端是否接收到推送的数据
...
def test_TestCase_AcuRev4100_WEB2_005_004_case24(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    # 选择 AcuAio 设备的 AI → AI 参数
    device_row = page.locator("tr, .el-table__row").filter(has_text="AcuAio").first
    if device_row.count() == 0:
        pytest.skip("测试环境未找到 AcuAio 设备，请检查设备连接")

    device_row.locator(".el-checkbox__inner").click()
    page.wait_for_timeout(500)

    # 展开参数组 AI
    param_section = page.locator(".el-collapse-item, tr").filter(has_text="AI").first
    if param_section.count() > 0:
        param_section.click()
        page.wait_for_timeout(300)

    # 勾选参数 AI
    param_row = page.locator("tr, .el-table__row").filter(has_text="AI").first
    ...
    # TODO: 启动 MQTT 订阅客户端，等待 60s，断言收到含 AI 参数的 JSON 消息
