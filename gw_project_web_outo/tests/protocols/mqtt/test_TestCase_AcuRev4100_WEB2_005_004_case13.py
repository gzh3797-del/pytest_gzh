...
# 用例编号: TestCase_AcuRev4100_WEB2_005_004_case13
# 用例标题: All Meters Use One Topic:所有设备使用一个主题推送，不使能时，客户端收不到数据，使能时，客户端可以接收到数据
...
#   5.All Meters Use One Topic选择使能，查看客户端是否可以接收到消息
#   6.Clean Session选择为true
#   7.All Meters Use One Topic选择不使能，查看客户端是否可以接收到消息
# 预期结果: 5.客户端可以接收到数据 | 7.客户端不可以接收到消息
...
@pytest.mark.xfail(strict=False, reason="产品 Topic and Parameter Selection 页未找到 'All Meters Use One Topic' 控件，控件名称或位置可能不同")
def test_TestCase_AcuRev4100_WEB2_005_004_case13(login_page: LoginPage):
    ...
    one_topic.locator(".el-radio, .el-switch, .el-checkbox").first.click()
    page.get_by_role("button", name="Save").click()
