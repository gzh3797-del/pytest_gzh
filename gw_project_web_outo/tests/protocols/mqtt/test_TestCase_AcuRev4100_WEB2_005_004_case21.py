...
# 用例编号: TestCase_AcuRev4100_WEB2_005_004_case21
# 用例标题: 下拉设备选择AcuRev-4100，选择Demand，选择参数需量，查看客户端是否接收到推送的数据
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3.MQTT服务端启动正常
# 测试步骤:
#   1.Communications->MQTT Device Parameter Config
#   2.下拉设备选择AcuRev-4100，选择Demand
#   3.遍历参数需量
#   4.查看客户端是否接收到推送的数据
#   5.选择全部的参数
#   6.查看客户端是否接收到推送的数据
...
def test_TestCase_AcuRev4100_WEB2_005_004_case21(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    # 选择 AcuRev-4100 设备的 Demand → 需量 参数
    ...
    # 展开参数组 Demand
    param_section = page.locator(".el-collapse-item, tr").filter(has_text="Demand").first
    if param_section.count() > 0:
        param_section.click()
        page.wait_for_timeout(300)

    # 勾选参数 需量
    param_row = page.locator("tr, .el-table__row").filter(has_text="需量").first
    ...
    # TODO: 启动 MQTT 订阅客户端，等待 60s，断言收到含 需量 参数的 JSON 消息
