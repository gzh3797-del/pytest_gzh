...
# 用例编号: TestCase_AcuRev4100_WEB2_005_004_case07
# 用例标题: Clean Session是否清理会话，为False时，设置服务质量为QoS1/2下次重新连接会再次将断开期间未收到的消息发送给客户端
...
#   4.Clean Session选择为false
#   5.点击save，提示保存成功
#   6.断开客户端连接，等待一段时间，重新连接后查看客户端接收到了断链期间接收到的数据
# 预期结果: 5.提示保存成功 | 6.客户端重新连接后可以接收到断链期间的数据
