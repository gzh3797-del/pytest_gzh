...
# 用例编号: TestCase_AcuRev4100_WEB2_005_004_case23
# 用例标题: 下拉设备选择AcuRev-4100，选择参数DI statue,DI Counter,RO Statue,DO Statue，查看客户端是否接收到推送的数据
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3.MQTT服务端启动正常
# 测试步骤:
#   1.Communications->MQTT Device Parameter Config
#   2.下拉设备选择AcuRev-4100，选择DI statue
#   3.遍历参数DI statue,DI Counter,RO Statue,DO Statue
#   4.查看客户端是否接收到推送的数据
#   5.选择全部的参数
#   6.查看客户端是否接收到推送的数据
...
def test_TestCase_AcuRev4100_WEB2_005_004_case23(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    # 选择 AcuRev-4100 设备的 DI/RO → DI 参数
    device_row = page.locator("tr, .el-table__row").filter(has_text="AcuRev-4100").first
    if device_row.count() == 0:
        pytest.skip("测试环境未找到 AcuRev-4100 设备，请检查设备连接")

    device_row.locator(".el-checkbox__inner").click()
    page.wait_for_timeout(500)

    # 展开参数组 DI/RO
    param_section = page.locator(".el-collapse-item, tr").filter(has_text="DI/RO").first
    if param_section.count() > 0:
        param_section.click()
        page.wait_for_timeout(300)

    # 勾选参数 DI
    param_row = page.locator("tr, .el-table__row").filter(has_text="DI").first
    ...
    # TODO: 启动 MQTT 订阅客户端，等待 60s，断言收到含 DI 参数的 JSON 消息
