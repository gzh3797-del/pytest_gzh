...
# 用例编号: TestCase_AcuRev4100_WEB2_008_003_case12
# 用例标题: 开启SNMP，配置Version为SNMPv3、端口为16100、Username和Password、Auth Protocol为MD5、Privacy Protocol为DES和Privacy Password与管理端一致
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3、NMS管理端已开启
# 测试步骤:
#   1.登录Setting-Communications-SNMP页面
#   2.开启SNMP，配置【Version：SNMPv3】、【端口：16100】、【Username：admin】、【Password：12345678】、【Auth Protocol:MD5】、【Privacy Protocol：DES】和【Privacy Password：12345678】与NMS管理端一致
...
def test_TestCase_AcuRev4100_WEB2_008_003_case12(login_page: LoginPage):
    login_page.open()
    login_page.login()
