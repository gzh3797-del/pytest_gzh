...
# 用例编号: TestCase_AcuRev4100_WEB2_008_003_case06
# 用例标题: 开启SNMP，配置Version为SNMPv2C、端口为161与管理端一致，RO Community与管理端不一致
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3、NMS管理端已开启
# 测试步骤:
#   1.登录Setting-Communications-SNMP页面
#   2.开启SNMP，配置Version为SNMPv2C、端口为16199，RO Community为public，点击保存
#   3.点击Download MIB File，下载MIB文件
#   4.NMS管理端导入下载的MIB文件，配置端口为与WEB2一致，RO Community为123456
#   5.NMS管理端GET请求接入设备的数据
#   6.使用IPV6登录网页，NMS管理端配置IPV6地址，重复1-5步骤
# 预期结果: 2.参数保存成功 | 3.下载文件格式和内容正确，符合SNMP标准协议 | 5.请求设备信息超时
...
def test_TestCase_AcuRev4100_WEB2_008_003_case06(login_page: LoginPage):
    login_page.open()
    login_page.login()
    page = login_page.page
    _nav_protocol(page, "SNMP")

    # 开启 SNMP，配置 Version=v2c, Port=161, Community='wrong_community'
    ...
    community_field.fill("wrong_community")

    page.get_by_role("button", name="Save").click()
    page.wait_for_timeout(1000)
    assert page.locator(".el-message--error").count() == 0, "保存不应出现错误提示"
    # TODO: 使用 pysnmp 从 NMS 端验证 SNMP MIB 数据失败
