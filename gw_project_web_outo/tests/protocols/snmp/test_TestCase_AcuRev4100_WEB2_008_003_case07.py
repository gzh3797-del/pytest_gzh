...
# 用例编号: TestCase_AcuRev4100_WEB2_008_003_case07
# 用例标题: 开启SNMP，配置Version为SNMPv2C、端口与管理端不一致
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3、NMS管理端已开启
# 测试步骤:
#   1.登录Setting-Communications-SNMP页面
#   2.开启SNMP，配置Version为SNMPv2C、端口为161，RO Community为public，点击保存
#   3.点击Download MIB File，下载MIB文件
#   4.NMS管理端导入下载的MIB文件，配置端口为163，RO Community为public
...
def test_TestCase_AcuRev4100_WEB2_008_003_case07(login_page: LoginPage):
    login_page.open()
    login_page.login()
    page = login_page.page
    _nav_protocol(page, "SNMP")

    # 开启 SNMP，配置 Version=v2c, Port=16200, Community='public'
