...
# 用例编号: TestCase_AcuRev4100_WEB2_008_003_case02
# 用例标题: 开启SNMP，配置Version为SNMPv2C、端口为16100、RO Community与管理端一致
# 预置条件: 1、服务启动正常，账号登录成功 | 2、环境存在多个设备 | 3、NMS管理端已开启
# 测试步骤:
#   1.登录Setting-Communications-SNMP页面
#   2.开启SNMP，配置Version为SNMPv2C、端口为16100，RO Community为private，点击保存
...
def test_TestCase_AcuRev4100_WEB2_008_003_case02(login_page: LoginPage):
    login_page.open()
    login_page.login()
    page = login_page.page
    _nav_protocol(page, "SNMP")

    # 开启 SNMP，配置 Version=v2c, Port=16100, Community='public'
