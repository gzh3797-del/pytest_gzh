...
# 用例编号：TestCase_AcuHMI_003_02_case11
# 用例标题：Logger2 LogFileNamePrefix超过20字符保存失败
# 预置条件：
#   1. 服务启动正常，账号登录成功
# 测试步骤：
#   1. Logger2开关为enable，PostChannel选择Channel3，LogFileFormat选择Json，
...
def test_TestCase_AcuHMI_003_02_case11(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    prefix_input.fill("meter2_logger1_123456")  # 21 chars — 超出20字符限制
    page.wait_for_timeout(300)
    ...
    assert form_errors > 0 or msg_errors > 0, (
        "Logger2 LogFileNamePrefix超过20字符时保存应失败并显示错误提示，"
