# 用例编号: TestCase_AcuHMI_005_04_case04_2
# 用例标题: Email Server=192.168.1.1，Port=65534，TLS=OFF，保存成功
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 配置 Email Server=192.168.1.1, Email Port=65534, TLS/SSL=OFF, 其他字段使用有效基准值
#   3. 点击 Save
# 预期结果: 保存成功
...
def test_TestCase_AcuHMI_005_04_case04_2(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    page.get_by_label("Email Server", exact=False).fill("192.168.1.1")
    page.get_by_label("Email Port", exact=False).fill("65534")
    try:
        page.locator(".el-radio").filter(has_text="Off").click()
        page.wait_for_timeout(200)
    except Exception:
