# 用例编号: TestCase_AcuHMI_005_04_case05_1
# 用例标题: 发件人姓名为纯数字(1233)，保存成功
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 填写基准配置，将 Sender Name 改为 1233（纯数字）
#   3. 点击 Save
# 预期结果: 保存成功
...
def test_TestCase_AcuHMI_005_04_case05_1(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    page.get_by_label("Sender Name", exact=True).fill("1233")
