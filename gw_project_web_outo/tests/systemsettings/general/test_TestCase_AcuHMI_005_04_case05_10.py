# 用例编号: TestCase_AcuHMI_005_04_case05_10
# 用例标题: 用户名超过40字符，保存失败
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 填写基准配置，将 Username 改为41个字符（超出最大长度限制）
#   3. 点击 Save
# 预期结果: 保存失败，显示表单校验错误
...
def test_TestCase_AcuHMI_005_04_case05_10(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    page.get_by_label("Username", exact=True).fill("q" * 41)
