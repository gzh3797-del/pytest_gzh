# 用例编号: TestCase_AcuHMI_005_04_case05_4
# 用例标题: 发件人姓名/From Email/密码均超过40字符，保存失败
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 填写基准配置，将 Sender Name、From Email Address、Password 均填入41个字符
#   3. 点击 Save
# 预期结果: 保存失败，显示表单校验错误（超出最大40字符限制）
...
def test_TestCase_AcuHMI_005_04_case05_4(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    long_str = "q" * 41
    page.get_by_label("Sender Name", exact=True).fill(long_str)
    page.get_by_label("From Email Address", exact=True).fill(long_str)
    page.get_by_label("Password", exact=True).fill(long_str)
