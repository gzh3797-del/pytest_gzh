# 用例编号: TestCase_AcuHMI_005_04_case04
# 用例标题: 电子邮件服务器IP=192.168.1.254，端口=1，TLS=AUTO，保存成功
# 预置条件: 管理权限登录AcuHMI网页
# 测试步骤:
#   1. 进入 System Settings -> Email
#   2. 配置 Email Server=192.168.1.254, Email Port=1, TLS/SSL=AUTO, 其他字段使用有效基准值
#   3. 点击 Save
# 预期结果: 保存成功（IP不可达但配置可保存）
...
def test_TestCase_AcuHMI_005_04_case04(login_page: LoginPage):
    login_page.open()
    login_page.login()
    ...
    page.get_by_label("Email Server", exact=False).fill("192.168.1.254")
    page.get_by_label("Email Port", exact=False).fill("1")
    try:
