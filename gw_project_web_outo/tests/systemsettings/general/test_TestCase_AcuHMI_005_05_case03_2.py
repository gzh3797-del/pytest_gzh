...
# 用例编号：TestCase_AcuHMI_005_05_case03_2
# 用例标题：收件人3：配置后Test Email发送成功
# 预置条件：管理权限登录AcuHMI，有可用邮件服务器，已配置3个收件人
# 测试步骤：配置3个收件人邮件地址，Enable，Save，Test Email
# 预期结果：Test Email发送成功，收件人收到邮件
@pytest.mark.xfail(strict=False, reason="Test Email需要真实可用的SMTP邮件服务器")
def test_TestCase_AcuHMI_005_05_case03_2(login_page):
    login_page.open()
    login_page.login()
    ...
    try:
        ...
        pass
    try:
        page.locator(".el-form-item").filter(has_text="Recipient 3").locator("input").first.fill("Recipient3@163.com")
    except Exception:
        pass
