...
def _dismiss_dialog(page):
    """关闭可能出现的 el-message-box 弹窗（Warning / Confirm / Info）"""
    overlay = page.locator(".el-overlay.is-message-box")
    if overlay.count() == 0:
        return
    # 优先点击 OK / Confirm / Yes 等确认按钮
    for btn_name in ["OK", "Ok", "Confirm", "Yes", "确认", "确定"]:
        ...
        if btn.count() > 0 and btn.first.is_visible():
            ...
            return
    # 备选：点击任意按钮关闭
    btns = overlay.locator(".el-button").all()
    for btn in btns:
        try:
            if btn.is_visible():
                btn.click()
...
def test_TestCase_AcuHMI_005_01_case04(login_page: LoginPage):
    ...
    for idx, tz_display in enumerate(all_timezones[:10], 1):
