...
回填列：Excel 第 21 列「需补充信息(claude识别回填)」
用户答复列：Excel 第 22 列「用户答复(基于需补充信息，澄清信息)」
...
3. 初始化 Excel 第 21、22 列（首次运行时自动追加）
...
- 只写入第 21、22 列，**不修改**原始用例数据（第 1-20 列）
- 已标注深绿色的行，二次运行时**不会被覆盖**（幂等保护）
- 「自动化」列（第16列）值不为"是"的用例**跳过不分析**
...
- struct.md 存在但该模块用例描述模糊 → 在第22列填写用户答复，重新调用
