...
回填列：Excel Q 列「需补充信息(claude识别回填)」
用户答复列：Excel R 列「用户答复(基于需补充信息,澄清信息)」
...
3. 确认 Excel Q 列（需补充信息）和 R 列（用户答复）存在
...
| `用户识别半自动化` | 是否半自动化（"是"/空） |
| `用户识别自动化` | 是否做自动化（"是"/空） |
| `需补充信息(claude识别回填)` | Claude 回填分析结果（Q 列） |
| `用户答复(基于需补充信息,澄清信息)` | 用户澄清内容（R 列） |
