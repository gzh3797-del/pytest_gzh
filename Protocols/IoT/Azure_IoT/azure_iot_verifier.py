...
用法：
    python Protocols/IoT/Azure_IoT/azure_iot_verifier.py
    python Protocols/IoT/Azure_IoT/azure_iot_verifier.py --timeout 120
    python Protocols/IoT/Azure_IoT/azure_iot_verifier.py --no-web --timeout 120
"""
import argparse
import asyncio
import html
...
try:
    ...
    from test_case.WEB2_4100.operation.AzureIoTPage import AzureIoTPage
    _SELENIUM_OK = True
except ImportError:
    _SELENIUM_OK = False
...
def _web_setup(cfg: dict, config_path: str = None):
    """登录网关，配置并启用 Azure IoT，返回 (driver, page) 供 teardown 复用。
    ...
    gw    = cfg['gateway']
    azure = cfg['azure_iot']
    cert_file = _abs_path(azure.get('cert_file', '')) if azure.get('cert_file') else ''
    key_file  = _abs_path(azure.get('key_file', ''))  if azure.get('key_file')  else ''
    if azure.get('enable_ssl'):
    ...
    try:
        ...
        page = AzureIoTPage(driver)
        page.navigate_to_azure_iot()
        result = page.configure(
            primary_conn_str=azure['primary_connection_string'],
            secondary_conn_str=azure.get('secondary_connection_string', ''),
            interval=azure.get('interval', '30 seconds'),
            enable_ssl=azure.get('enable_ssl', False),
            cert_file=cert_file,
            key_file=key_file,
        )
        ...
        if page_devices:
            ...
            if added or removed:
                ...
                cfg['azure_iot']['expected_devices'] = updated
...
def _web_teardown(driver, page) -> None:
    try:
        page.navigate_to_azure_iot()
        page.disable()
        logging.info("[Web] Azure IoT 已禁用")
...
# Azure IoT 上报设备名 → Protocols/devices/ 模块名
...
def _parse_modules(payload: dict) -> list[dict]:
    """解析 JSON payload，返回 [{'name': str, 'reading': list, ...}]"""
...
class _EventHubReceiver:
    """从 Azure IoT Hub 内置 EventHub 端点接收设备消息。"""
    ...
    def start(self, on_payload, start_time):
        """在后台线程启动接收。on_payload(payload_dict) 在消息到达时被调用。"""
        ...
        def _run():
            try:
                ...
                logging.debug(f"EventHub receive 结束：{e}")
...
_CSS = """
  ...
  .card.warn .num{color:#ffc107}.card.total .num{color:#0078d4}
  ...
  details.section>summary{list-style:none;cursor:pointer;padding:12px 16px;
    background:#f0f4f8;border-left:4px solid #0078d4;font-size:15px;font-weight:bold;
    color:#0078d4;display:flex;align-items:center;gap:8px;user-select:none}
...
def _gen_device_list_section(reported: set[str], expected: set[str]) -> str:
    ...
    badge = (_badge('一致', 'ok') if ok
             else _badge('不一致', 'err') if missing_d else _badge('多余设备', 'warn'))
    rows = ""
    ...
    for dev_name in sorted(reported):
        ...
        for i, param in enumerate(r['_all_params']):
            ...
            if param in miss_set:
                ...
                bg, st = '#e8f4fd', '<span style="color:#0078d4">+ 多余</span>'
...
<table>
  ...
  <tbody>{rows}</tbody>
    ...
    ob = (_badge('全部通过', 'ok') if total_fail == 0 and total_warn == 0
          else _badge(f'{total_fail} 个失败', 'err') if total_fail
          else _badge(f'{total_warn} 个跳过', 'warn'))
...
def _gen_value_section(value_results: dict, tol_pct: float, tol_abs: float) -> str:
...
def generate_html_report(
    ...
    # 连接字符串只取前 60 字符作为摘要，避免报告显示过长
    conn_summary = config_info.get('eventhub_connection_string', '')[:60] + '…'
...
<div class="meta">
  ...
  EventHub：{html.escape(conn_summary)} &nbsp;|&nbsp;
  Consumer Group：{html.escape(config_info.get('consumer_group','$Default'))} &nbsp;|&nbsp;
...
def run_verifier(cfg: dict, timeout: int = 90, skip_web: bool = False,
    ...
    try:
        if not skip_web:
            logging.info("=" * 60)
            logging.info("[Web] 启动浏览器，配置 Azure IoT…")
    ...
    try:
        ...
        if _driver is not None:
            logging.info("[Web] 禁用 Azure IoT 并关闭浏览器…")
...
def _run_verifier_core(cfg: dict, timeout: int) -> bool:
    ...
    eh_conn  = azure['eventhub_connection_string']
    cg       = azure.get('consumer_group', '$Default')
    ...
    def on_payload(payload: dict, raw_body: str):
        msg_count[0] += 1
        preview = raw_body[:200] + ('…' if len(raw_body) > 200 else '')
    ...
    # 从当前时间稍前开始接收，防止漏掉刚推送的消息
    start_time = datetime.now(timezone.utc) - timedelta(seconds=10)
    receiver = _EventHubReceiver(eh_conn, cg)
    logging.info(f"正在连接 Azure IoT Hub EventHub 端点…")
    logging.info(f"Consumer Group：{cg}")
    t = receiver.start(on_payload, start_time)
    ...
    if not raw_messages:
        print("\n[FAIL] 超时内未收到任何消息，请检查 EventHub 连接字符串和网关配置")
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Azure IoT 消息验证工具')
