# -*- coding: utf-8 -*-
"""
devices/acuiom03.py — AcuIOM-03 Modbus 参数地址映射

型号规格：14 DI + 2 DO + 2 RO
地址来源：AcuIOM Modbus Address Table v1.01

DI Status（FC 02H，bit）：0x0000–0x000D（DI 1–14）
DO Status（FC 01H，bit）：0x0000–0x0001（DO 1–2）
RO Status（FC 01H，bit）：0x0020–0x0021（RO 1–2）
DI Pulse Count（FC 03H，uint32）：0x2200–0x2217（DI 1–14，每项 2 寄存器）
...
def build_param_map() -> dict[str, ModbusRegister]:
    """AcuIOM-03 param_key → ModbusRegister 映射。"""
    regs: dict[str, ModbusRegister] = {}

    # ── DI Status（FC 02H，bit，addr 0x0000–0x000D） ─────────────────────────
    for i in range(1, 15):
        key = f"DI_{i}_Status"
        regs[key] = ModbusRegister(key, i - 1, 'bit', fc=2)

    # ── DO Status（FC 01H，bit，addr 0x0000–0x0001） ──────────────────────────
    for i in range(1, 3):
        key = f"DO{i}_Status"
    ...
    # ── DI Pulse Count（FC 03H，uint32，addr 0x2200–0x2217，步长 2） ──────────
    for i in range(1, 15):
