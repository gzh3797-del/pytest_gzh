...
报告输出到 `Protocols/reports/` 目录，文件名含时间戳。
...
python -X utf8 Protocols/MQTT/mqtt_comparator.py

# 快照模式，指定设备
python -X utf8 Protocols/MQTT/mqtt_comparator.py --device acurev4100

# 快照模式，指定文件
python -X utf8 Protocols/MQTT/mqtt_comparator.py --file Protocols/MQTT/4100data.json

# 实时采集，单设备（等待 60s）
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live

# 实时采集，单设备，自定义超时
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --timeout 120

# 实时采集，多设备合并报告（推荐，一次生成全部设备报告）
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --all-modules --timeout 180

# 只做范围和单位检查，跳过 Modbus 数值比对
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --all-modules --timeout 180 --no-modbus
```
...
所有连接参数在 `Protocols/config.py` 中统一配置：
...
# 多网卡 / 多 IP
python Protocols/MQTT/gen_certs.py --host 192.168.2.61 192.168.2.62

# 自定义有效期（默认 3650 天 = 10 年）
python Protocols/MQTT/gen_certs.py --host 192.168.2.61 --days 365
```

> **注意**：若未指定 `--host`，脚本会打印警告提示。如需设备通过本机非回环地址连入，必须在 SAN 中包含该 IP。

证书文件输出到 `Protocols/MQTT/certs/`：
...
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --all-modules --ssl

# 指定自己的证书目录（适用于多人共用测试台、各自 IP 不同的场景）
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --all-modules --ssl --cert-dir C:\my_certs

# 显式指定端口
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --all-modules --ssl --port 8883

# 单设备实时采集 + mTLS
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --ssl
```
...
python Protocols/MQTT/gen_certs.py --host 192.168.2.100 --out C:\certs_user1

# 第二步：运行时通过 --cert-dir 传入
python -X utf8 Protocols/MQTT/mqtt_comparator.py --live --all-modules --ssl --cert-dir C:\certs_user1
```
...
当不传 `--cert-dir` 或单个路径参数时，脚本从 `Protocols/config.py` 读取默认路径：
