...
async def _start_broker(host: str, port: int, ssl_config: Optional[dict] = None):
    ...
    if ssl_config:
        # amqtt 0.11.x 默认 verify_mode = CERT_OPTIONAL（不强制要求客户端证书）。
        # 保持默认值：Broker 端单向 TLS（验证服务器证书），设备侧无需出示客户端证书。
        # 若设备 SSL 实现支持 mTLS 可改为 CERT_REQUIRED；当前保持 CERT_OPTIONAL 确保兼容性。
        pass
...
async def _collect_modules_data(
    ...
    threading.Thread(target=lambda: asyncio.run(_broker_coro()), daemon=True).start()
    ...
    if ssl_config:
        # paho 内部订阅走 SSL 连接到本机 Broker。
        # PROTOCOL_TLS_CLIENT 默认 check_hostname=True，连接目标 127.0.0.1 时
        # 会校验服务器证书 SAN 中是否包含 127.0.0.1（gen_certs.py 已写入）。
        # 注意：若证书 SAN 不含本机 IP，会在此抛出 SSLCertVerificationError。
        sub.tls_set(
    ...
    while loop.time() < deadline and not _live_stop_event.is_set():
        ...
        for mod_name, mod_model, online, value_map, unit_map in pairs:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 📥 [{mod_name}]  "
                  f"第 {md['push_count']} 次推送  间隔 {interval_str}  累计参数 {len(md['value_map'])}")
            log.info("📥 [%s] 第 %d 次推送  间隔 %s  累计 %d 个参数",
...
async def collect_live_mqtt(
    ...
    modules_data = await _collect_modules_data(host, port, topic, timeout, ssl_config=ssl_config)
