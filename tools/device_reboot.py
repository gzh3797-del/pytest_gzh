...
# 注：原 `from test_case.AcuRev1320...` 的 HandleMemory 随 test_case/ 移除已失效，
# 仅 __main__ 计时保持演示用到，迁移后由组员按需在 projects/ 内重建。
...
if __name__ == '__main__':
    ...
    pow_on_device()
    time.sleep(5)
    # rm.read_sys_time() / rm.close_rtu_client()：HandleMemory 随 test_case/ 移除，迁移后由组员重建
    pow_off_device()
