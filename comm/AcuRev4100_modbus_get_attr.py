...
def read_system_frequency(conn_mode):
    ...
    voltage: list = client.read_measurement(address=8192, count=2, device_id=1)
    client.close()
...
def read_frequency(standard_value, times=1):
    ...
    for v in range(times):
        value = mes.read_measurement(address=8192, count=2, device_id=1)
...
def Read_Phase_A_Voltage(standard_value, times=1):
    ...
    val_list = []
    for v in range(times):
        value = mes.read_measurement(address=8194, count=2, device_id=1)
...
def Read_Phase_B_Voltage(standard_value, times=1):
    ...
    val_list = []
    for v in range(times):
        value = mes.read_measurement(address=8196, count=2, device_id=1)
...
def Read_Phase_C_Voltage(standard_value, times=1):
    ...
    val_list = []
    for v in range(times):
        value = mes.read_measurement(address=8198, count=2, device_id=1)
