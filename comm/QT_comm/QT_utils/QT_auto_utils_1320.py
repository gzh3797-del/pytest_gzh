...
class AutoHelper:
    ...
    def check_image_exists(self, image_path: str, timeout: int = 5, confidence: float = 0.8) -> bool:
        """
        检查页面中是否存在指定图片
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if location:
                    ...
                    # print(f"✅ 找到图片: {os.path.basename(full_image_path)}")
                    return True
            except Exception as e:
                pass
            time.sleep(1)
        ...
        # print(f"⚠️ 未找到图片: {os.path.basename(full_image_path)}")
        return False
    ...
    def paste_text(self, text: str, delay: float = 0.1):
        """
        将文本复制到剪贴板并粘贴
